...
qx.Class.define("inspector.console.ConsoleView",
{
  ...
  construct : function(console)
  {
    ...
    var leadingLabel = new qx.ui.basic.Label("<b style='color: darkblue'>>></b>");
    leadingLabel.setRich(true);
    ...
    inputComposite.add(this._inputTextField, {
      flex : 1
    });
  ...
  members :
  {
    ...
    _process : function(text)
    {
      ...
      try
      {
        ...
        if (object != null)
        {
          this._objectFolder[this._objectFolderIndex] =
          {
            name : text,
            object : object
          };
    ...
    _keyDownHandler : function(e)
    {
      ...
      if (e.getKeyIdentifier() == "Up")
      {
        ...
        if (!this._autoCompletePopup.isOnScreen()) {
          ...
          this._autoCompletePopup.previousRow();
        }
        return;
      }
      ...
      if (e.getKeyIdentifier() == "Down")
      {
        ...
        if (!this._autoCompletePopup.isOnScreen()) {
          ...
          this._autoCompletePopup.nextRow();
        }
        return;
      }
    ...
    */
    ...
    _printReturnValue : function(returnValue)
    {
      ...
      } else if (returnValue instanceof iFrameWindow.Array)
      {
        ...
        var label = this._getLabel("<span class='ins_console_link' onclick='" + "qx.core.Init.getApplication().inspectObjectByInternalId(" + this._objectFolderIndex + ")" + "'>" + arrayRepresentation + "</span>", "ins_console_return_array");
        this._content.setHtml(this._content.getHtml() + label);
        return;

        // check for objects
      } else if (iFrameWindow && (returnValue instanceof iFrameWindow.Object || returnValue == iFrameWindow.window || returnValue == iFrameWindow.document))
      {
        // if yes, print out that it is one
        var label = this._getLabel("<span class='ins_console_link' onclick='" + "qx.core.Init.getApplication().inspectObjectByInternalId(" + this._objectFolderIndex + ")" + "'>" + returnValue + " </span>", "ins_console_return_object");
        this._content.setHtml(this._content.getHtml() + label);
        return;
    ...
    _printQxObject : function(object)
    {
      ...
      var label = this._getLabel("<span class='ins_console_link' onclick=" + "\"qx.core.Init.getApplication().setWidgetByHash('" + object.toHashCode() + "', 'console');\"> " + object.classname + " [" + object.toHashCode() + "]</span> " + "<span class='ins_console_dom_link' onclick='" + "qx.core.Init.getApplication().inspectObjectByInternalId(" + this._objectFolderIndex + ")" + "'>inspect Object</span>", "ins_console_return_qxobject");
    ...
    */
    ...
    _getLabel : function(text, clazz, icon)
    {
      ...
      {
        var imageURI = qx.util.ResourceManager.getInstance().toUri("inspector/images/shell/" + icon + "Icon.png");
