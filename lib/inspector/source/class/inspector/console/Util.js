    ...
    evalOnIframe : function(code)
    {
      ...
      try
      {
        if (qx.core.Environment.get("engine.name") == "opera" && qx.core.Environment.get("engine.version") < 9.8) {
          ...
          var returnCode = "";
          if (qx.core.Environment.get("engine.name") == "webkit" || qx.core.Environment.get("engine.name") == "opera" || (qx.core.Environment.get("engine.name") == "mshtml" && parseFloat(qx.core.Environment.get("browser.version")) >= 9) || (qx.core.Environment.get("engine.name") == "gecko" && parseFloat(qx.core.Environment.get("browser.version")) >= 4)) {
            returnCode = "return eval('" + code + "');"
          } else {
            returnCode = "return eval.call(window, '" + code + "');"
          }
          iFrameWindow.qx.lang.Function.globalEval(["window.top.inspector.$$inspector = function()", "{", "  try {", returnCode, "  } catch (ex) {", "    return ex;", "  }", "};"].join(""));
