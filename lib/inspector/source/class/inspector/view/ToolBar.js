...
qx.Class.define("inspector.view.ToolBar",
{
  ...
  construct : function(inspectorModel, state)
  {
    ...
    //var inspectorLabel = new qx.ui.basic.Label("Inspector");

    //inspectorLabel.setRich(true);

    //inspectorLabel.setPaddingLeft(10);

    //inspectorLabel.setPaddingRight(5);

    //inspectorLabel.setPadding(0,0,0,10);
    ...
    // reload button
    this._reloadButton = new qx.ui.toolbar.Button(null, "qx/icon/Tango/16/actions/view-refresh.png");
    ...
    this._reloadButton.addListener("execute", function(e)
    {
      ...
      this._widgetsButton.setValue(false);
      this._consoleButton.setValue(false);
      this._propertyButton.setValue(false);
      this._seleniumButton.setValue(false);
    }, this);
  ...
  members :
  {
    ...
    __createWindow : function(name, clazz, buttonRef)
    {
      ...
      var button = this[buttonRef] = new qx.ui.toolbar.CheckBox(name, "source/Modern/checkbox.png");
      button.setAppearance(null);
      ...
      button.addListener("changeValue", function(e) {
        ...
        if (e.getData())
        {
          button.setIcon("source/Modern/checkbox-checked.png");
          win.open();
        } else
        {
          button.setIcon("source/Modern/checkbox.png");
          win.close();
        }
      }, this);
    ...
    __changeInspected : function(e)
    {
      ...
      if (object != null)
      {
        this._selectedWidgetLabel.setValue("<tt>" + object.classname + "[" + object.toHashCode() + "]</tt>");
  ...
  destruct : function()
  {
    ...
    this._separator = this._inspectButton = this._selectedWidgetLabel = this._urlTextField = this._reloadButton = this._objectsButton = this._widgetsButton = this._propertyButton = this._consoleButton = this._consoleWindow = this.__overflowMenu = this._seleniumButton = null;
  }
});
