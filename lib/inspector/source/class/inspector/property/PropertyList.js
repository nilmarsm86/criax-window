...
qx.Class.define("inspector.property.PropertyList",
{
        ...
        if (replace)
        {
          ...
          for (var key in properties[i]) {
            ...
            if (properties[i][key].group == null)
            {
              ...
              this._propertyRows[classnames[i][key] + "." + key] =
              {
      ...
      for (; (classnames.length - 1) * 2 < this.getChildren().length; )
      {
        ...
        for (var currentIndex = 0; currentIndex < children.length; currentIndex++) {
          if (children[currentIndex].classname == "qx.ui.basic.Label")
          {
            ...
            var classKey = children[currentIndex].getUserData("classname") + "." + children[currentIndex].getUserData("key");
    ...
    _getPropertyWidgetFull : function(propertySet, key, classname)
    {
      ...
      if (propertySet.check !== null) {
        ...
        } else if (propertySet.check instanceof Array)
        {
          ...
          // text field
        } else if (propertySet.check == "Integer" || propertySet.check == "String" || propertySet.check == "NonEmptyString" || propertySet.check == "Label" || propertySet.check == "Float" || propertySet.check == "Double" || propertySet.check == "Number")
        {
    ...
    _setPropertyValueFull : function(key, classname, keepArrow)
    {
      ...
      } else if (layout.getCellWidget(row, 2).classname == "qx.ui.basic.Label")
      {
        if (value != null)
        {
          ...
          } else if ((property.check == "qx.ui.core.Widget") && (this._controller.getQxObject() instanceof qx.application.AbstractGui))
          {
    ...
    */
    ...
    __onPropertyTap : function(e)
    {
      ...
      if (this._propertyRows[classKey] != undefined)
      {
        ...
        this._arrow.container.add(this._arrow.arrow,
        {
          row : this._arrow.row,
          column : 0
        });
  ...
  */
  destruct : function()
  {
    this._propertyRows = this._comboBoxPopups = this._colorPopup = this._colorFields = this._oldPropertyListPool = null;
