    ...
    getQxhLocatorStep : function(widget, autWin)
    {
      ...
      for (var i = 0, l = children.length; i < l; i++) {
        ...
        if (children[i] == widget && childIndex === null) {
         ...
         else if (autWindow.qx.Class.getByName(children[i].classname) && widget instanceof autWindow.qx.Class.getByName(children[i].classname)) {
          twin = true;
        }// ditto if the child is of the same class as the target
         else if (autWindow.qx.Class.getByName(widget.classname) && children[i] instanceof autWindow.qx.Class.getByName(widget.classname)) {
          twin = true;
