 ...
 * @asset(qx/icon/Tango/16/actions/media-record.png)
...
qx.Class.define("inspector.selenium.View",
{
  ...
  {
    this.base(arguments);
    this.__seleniumScripts = ["/core/scripts/xmlextras.js", "/core/lib/prototype.js", "/core/lib/cssQuery/cssQuery-p.js", "/core/lib/snapsie.js", "/core/scripts/htmlutils.js", "/core/scripts/ui-element.js", "/core/scripts/selenium-browserdetect.js", "/core/scripts/selenium-browserbot.js", "/core/scripts/find_matching_child.js", "/core/scripts/selenium-api.js", "/core/scripts/selenium-commandhandlers.js", "/core/scripts/selenium-executionloop.js", "/core/scripts/selenium-remoterunner.js", "/core/scripts/selenium-logging.js", "/core/scripts/selenium-version.js", "/core/xpath/util.js", "/core/xpath/xmltoken.js", "/core/xpath/dom.js", "/core/xpath/xpath.js", "/core/xpath/javascript-xpath-0.1.11.js"];
    ...
    this.__getToolbarPart4();
    this.__toolbarButtonsState(false);

    // Options window
    this._optionsWindow = new inspector.selenium.OptionsWindow("Selenium core", "qx/icon/Tango/16/categories/system.png", this);
    ...
    logContainer.add(this._logArea, {
      flex : 1
    });
  ...
  members :
  {
    ...
    __getToolbarPart1 : function()
    {
      ...
      var addButton = new qx.ui.toolbar.Button(null, "icon/16/actions/list-add.png");

      //part1.add(addButton);

      //addButton.setEnabled(false);
      this._toolbar.add(addButton);
      addButton.addListener("execute", this.__addLocator, this);
      addButton.setToolTipText("Add locator for inspected widget");
      var removeButton = new qx.ui.toolbar.Button(null, "icon/16/actions/list-remove.png");

      //removeButton.setEnabled(false);

      //part1.add(removeButton);
      this._toolbar.add(removeButton);
      removeButton.addListener("execute", this.__removeSelectedRows, this);
      removeButton.setToolTipText("Remove selected command(s)");

      //clear all commands
      var clearButton = new qx.ui.toolbar.Button(null, "qx/icon/Tango/16/places/user-trash.png");
      clearButton.setToolTipText("Remove all commands");
      clearButton.addListener("execute",function(event){
        this.clearTable();
      },this);
      this._toolbar.add(clearButton);

      //return part1;
      return new qx.ui.toolbar.Separator();
    },
    ...
    __getToolbarPart2 : function()
    {
      ...
      this._toolbar.add(this._speedSlider);
    ...
    __getToolbarPart3 : function()
    {
      ...
      var runCmdButton = new qx.ui.toolbar.Button(null, "icon/16/actions/media-playback-start.png");
      this._toolbar.add(runCmdButton)
      runCmdButton.addListener("execute", this.runSeleniumCommands, this);
      runCmdButton.setToolTipText("Run selected command(s)");
      this._recordButton = new qx.ui.toolbar.CheckBox(null, "icon/16/actions/media-record.png");
      this._toolbar.add(this._recordButton);
      var recOpts = {
        converter : function(data) {
          return data ? "Stop adding commands for inspected widgets" : "Automatically add a new command for each inspected widget";
        }
      };
      this._recordButton.bind("value", this._recordButton, "toolTipText", recOpts);
      this._exportButton = new qx.ui.toolbar.CheckBox(null, "icon/16/actions/window-new.png");
      this._toolbar.add(this._exportButton);
    ...
    __getToolbarPart4 : function()
    {
      ...
      this._optionsButton = new qx.ui.toolbar.Button(null, "icon/16/categories/system.png");
      this._optionsButton.setToolTipText("Options");
      this._toolbar.add(this._optionsButton);
    ...
    __getLogControls : function()
    {
      ...
      var logContainerInner = new qx.ui.container.Composite(new qx.ui.layout.HBox());
      logContainerInner.setPaddingLeft(5);

      var btnClear = new qx.ui.form.Button(null,"qx/icon/Tango/16/places/user-trash.png");
      //btnClear.setDecorator(null);
      btnClear.addListener("execute", function(ev) {
        this._logArea.setHtml("")
      }, this);
      logContainerInner.add(btnClear);

      logContainerInner.add(new qx.ui.toolbar.Separator());
    ...
    clearTable : function() {
      ...
      {
        //var clear = widnow.confirm("Delete current test command list?");
        //if (clear) {
    ...
    _applySeleniumScripts : function(value, old)
    {
      ...
      //BOTON POR BOTON ACTIVAR O DESACTIVAR
      this.__toolbarButtonsState(false);

      //this._toolbar.getChildren()[0].setEnabled(false);//button add

      //this._toolbar.getChildren()[1].setEnabled(false);//button remove
      ...
      loader.addListenerOnce("finished", function(ev)
      {
        ...
        //alert(this.__userExt);
        if(this.__userExt == null){
          this.__userExt = seleniumCore + "/../script/user-extensions.js";
        }
    ...
    __scriptsLoaded : function(ev)
    {
      ...
      //ACTIVAR O DESACTIVAR BOTONES DEL TOOLBAR
      this.__toolbarButtonsState(true);
    ...
    /**
     * activar o desactivar botones de la barra
     * @param state {Boolean} active/desactive buttons
     * @private
     */
    __toolbarButtonsState : function(state)
    {
      this._toolbar.getChildren()[0].setEnabled(state);  //add button
      this._toolbar.getChildren()[1].setEnabled(state);  //remove button
      this._toolbar.getChildren()[3].setEnabled(state);  //slider
      this._toolbar.getChildren()[4].setEnabled(state);  //slider label
      this._toolbar.getChildren()[6].setEnabled(state);  //cmd button
      this._toolbar.getChildren()[7].setEnabled(state);  //checkbox
      this._toolbar.getChildren()[8].setEnabled(state);  //export button

      //iterar por cada hijo y si no es un separator entonces se desabilita
    }
  },
  destruct : function()
  {
