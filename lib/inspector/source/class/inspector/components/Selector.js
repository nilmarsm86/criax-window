...
qx.Class.define("inspector.components.Selector",
{
  ...
  members :
  {
    ...
    __onChangeApplication : function(e)
    {
      ...
      if (this.__applicationWindow.qx.core.Init.getApplication().getRoot().classname == "qx.ui.mobile.core.Root") {
    ...
    __createMobileHighlighter : function()
    {
      ...
      var rule = "." + cssClass + " {" + "outline: " + borderWidth + "px solid " + borderColor + ";" + "}";
    ...
    __searchWidget : function(widget, x, y)
    {
      ...
      for (var i = 0; i < widget._getChildren().length; i++)
      {
        ...
        {
          var win = this.__applicationWindow;
          if (qx.lang.Array.contains(excludes, childWidget) || (!this.__isMobileApp && win.qx.Class.isSubClassOf(childWidget.constructor, win.qx.ui.core.Spacer))) {
            continue;
          }
        }catch (ex) {
        }
        ...
        if (coordinates != null) {
          ...
          if (coordinates.right >= x && coordinates.left <= x && coordinates.bottom >= y && coordinates.top <= y) {
    ...
    {
      var win = this.__applicationWindow;
      if (win != null && this.__catchTapLayer != null && this.__highlighter != null)
      {
        this.__highlighter.hide();
