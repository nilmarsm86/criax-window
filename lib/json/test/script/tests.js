...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:tests.cb53d2489fa7.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="border-box",b="qx.bom.element.BoxSizing",c="css.boxsizing",d="",e="boxSizing",f="content-box",g=":",h=";";qx.Bootstrap.define(b,{statics:{__ep:{tags:{button:true,select:true},types:{search:true,button:true,submit:true,reset:true,checkbox:true,radio:true}},__eq:function(j){var i=this.__ep;return i.tags[j.tagName.toLowerCase()]||i.types[j.type];}
...
;if(qx.bom.Document.isStandardMode(qx.dom.Node.getWindow(m))){if(!this.__eq(m)){return f;}
;}
;return a;}
...
;}
;var bf=this.__jN;if(be!=bf){if(this.__jH){this.__jN=be;this.__jQ=this.__hJ(A,this.__jH,this.__jI,true);if(!this.__jQ){be=null;}
;}
;if(be!=bf){this.__jN=be;this.__hJ(A,this.__jI,this.__jH,false);}
;}
;}
...
,__jV:function(bo){while(bo&&bo.nodeType==1){if(bo.getAttribute(b)==B){return bo;}
;bo=bo.parentNode;}
;return null;}
,__jW:function(bp){while(bp&&bp.nodeType==1){if(bp.getAttribute(m)==B){return bp;}
;bp=bp.parentNode;}
;return null;}
,clearSession:function(){this.__gt.removeListener(this.__dd,K,this._onPointermove,this);this.__gt.removeListener(this.__dd,w,this._onPointerup,this,true);this.__gt.removeListener(this.__dd,i,this._onKeyDown,this,true);this.__gt.removeListener(this.__dd,D,this._onKeyUp,this,true);this.__gt.removeListener(this.__dd,E,this._onKeyPress,this,true);this.__gt.removeListener(this.__dd,k,this._onRoll,this,true);if(this.__jI){this.__hJ(G,this.__jI,this.__jH,false);}
;this.__jP=false;this.__jH=null;if(this.__jR){this.__jR.removeState(h);this.__jR=null;}
;this.__jI=null;this.__jO=false;this.__jS=null;this.__jT();}
...
;var bs=this.__jS?this.__jS.target:e.getTarget();var br=this.__jV(bs);if(br){this.__jI=br;var bt=qx.ui.core.Widget.getWidgetByElement(this.__jS.original);while(bt&&bt.isAnonymous()){bt=bt.getLayoutParent();}
;if(bt){this.__jR=bt;bt.addState(h);}
;if(!this.__hJ(x,this.__jI,this.__jH,true,e)){return false;}
;this.__gt.addListener(this.__dd,i,this._onKeyDown,this,true);this.__gt.addListener(this.__dd,D,this._onKeyUp,this,true);this.__gt.addListener(this.__dd,E,this._onKeyPress,this,true);this.__jO=true;return true;}
;}
,_onPointerdown:function(e){if(e.isPrimary()){this.__jS={target:e.getTarget(),original:e.getOriginalTarget(),left:e.getDocumentLeft(),top:e.getDocumentTop()};this.__gt.addListener(this.__dd,K,this._onPointermove,this);this.__gt.addListener(this.__dd,w,this._onPointerup,this,true);}
;}
,_onPointermove:function(e){if(!e.isPrimary()){return;}
;if(!this.__jO&&e.getPointerType()==o){var bA=this._getDelta(e);var bu=qx.event.handler.DragDrop.MIN_DRAG_DISTANCE;if(bA&&(Math.abs(bA.x)>bu||Math.abs(bA.y)>bu)){if(!this._start(e)){this.clearSession();return;}
;}
;}
;if(!this.__jO){return;}
;if(!this.__hJ(h,this.__jI,this.__jH,true,e)){this.clearSession();}
;var bv=e.getTarget();var by=this.getCursor();if(!by){by=qx.ui.core.DragDropCursor.getInstance();}
;var bx=by.getContentElement().getDomElement();if(bv!==bx){var bw=this.__jW(bv);if(bw&&bw!=this.__jH){if(this.__jH){this.__hJ(F,this.__jH,this.__jI,false,e);}
;this.__jP=true;this.__jH=bw;this.__jP=this.__hJ(H,bw,this.__jI,true,e);}
else if(!bw&&this.__jH){this.__hJ(F,this.__jH,this.__jI,false,e);this.__jH=null;this.__jP=false;qx.event.Timer.once(this.__jU,this,0);}
;}
;var bz=this.__jL;bz.Control=e.isCtrlPressed();bz.Shift=e.isShiftPressed();bz.Alt=e.isAltPressed();this.__jU();}
,_getDelta:function(e){if(!this.__jS){return null;}
;var bB=e.getDocumentLeft()-this.__jS.left;var bC=e.getDocumentTop()-this.__jS.top;return {"x":bB,"y":bC};}
,_onPointerup:function(e){if(!e.isPrimary()){return;}
;if(this.__jP&&this.__jQ){this.__hJ(l,this.__jH,this.__jI,false,e);}
;if(e.getTarget()==this.__jI){e.stopPropagation();}
;this.clearSession();}
,_onRoll:function(e){e.stop();}
,_onWindowBlur:function(e){this.clearSession();}
,_onKeyDown:function(e){var bD=e.getKeyIdentifier();switch(bD){case C:case q:case r:if(!this.__jL[bD]){this.__jL[bD]=true;this.__jU();}
;};}
,_onKeyUp:function(e){var bE=e.getKeyIdentifier();switch(bE){case C:case q:case r:if(this.__jL[bE]){this.__jL[bE]=false;this.__jU();}
;};}
,_onKeyPress:function(e){var bF=e.getKeyIdentifier();switch(bF){case g:this.clearSession();};}
},destruct:function(){qx.event.Registration.removeListener(window,a,this._onWindowBlur,this);this.__jI=this.__jH=this.__gt=this.__dd=this.__jJ=this.__jK=this.__jL=this.__c=null;}
...
(function(){var a="best-fit",b="placementRight",c="Boolean",d="bottom-right",e="' ",f="widget",g="placementLeft",h="qx.ui.core.MPlacement",i="left-top",j="Integer",k="left-middle",l="right-middle",m="top-center",n="[qx.ui.core.MPlacement.setMoveDirection()], the value was '",o="offsetRight",p="interval",q="keep-align",r="bottom-left",s="pointer",t="direct",u="shorthand",v="Invalid value for the parameter 'direction' ",w="offsetLeft",x="top-left",y="appear",z="offsetBottom",A="top",B="top-right",C="offsetTop",D="but 'top' or 'left' are allowed.",E="right-bottom",F="disappear",G="right-top",H="bottom-center",I="left-bottom",J="left";qx.Mixin.define(h,{statics:{__gF:null,__jX:J,setVisibleElement:function(K){this.__gF=K;}
,getVisibleElement:function(){return this.__gF;}
,setMoveDirection:function(L){if(L===A||L===J){this.__jX=L;}
else {throw new Error(v+n+L+e+D);}
;}
,getMoveDirection:function(){return this.__jX;}
},properties:{position:{check:[x,m,B,r,H,d,i,k,I,G,l,E],init:r,themeable:true},placeMethod:{check:[f,s],init:s,themeable:true},domMove:{check:c,init:false},placementModeX:{check:[t,q,a],init:q,themeable:true},placementModeY:{check:[t,q,a],init:q,themeable:true},offsetLeft:{check:j,init:0,themeable:true},offsetTop:{check:j,init:0,themeable:true},offsetRight:{check:j,init:0,themeable:true},offsetBottom:{check:j,init:0,themeable:true},offset:{group:[C,o,z,w],mode:u,themeable:true}},members:{__jY:null,__ka:null,__kb:null,getLayoutLocation:function(N){var P,O,R,top;O=N.getBounds();if(!O){return null;}
...
;}
,placeToWidget:function(bc,ba){if(ba){this.__kc();this.__jY=qx.lang.Function.bind(this.placeToWidget,this,bc,false);qx.event.Idle.getInstance().addListener(p,this.__jY);this.__kb=function(){this.__kc();}
...
;}
,__kc:function(){if(this.__jY){qx.event.Idle.getInstance().removeListener(p,this.__jY);this.__jY=null;}
...
(function(){var a="-",b="align-start",c="best-fit",d="qx.util.placement.Placement",e='__ke',f="middle",g="bottom",h="keep-align",i="align-end",j="align-center",k="Invalid 'mode' argument!'",l="center",m="edge-start",n="Class",o="direct",p="top",q="left",r="right",s="edge-end";qx.Class.define(d,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ke=qx.util.placement.DirectAxis;}
,properties:{axisX:{check:n},axisY:{check:n},edge:{check:[p,r,g,q],init:p},align:{check:[p,r,g,q,l,f],init:r}},statics:{__kf:null,compute:function(D,w,t,u,C,x,y){this.__kf=this.__kf||new qx.util.placement.Placement();var A=C.split(a);var z=A[0];var v=A[1];{var B;}
;this.__kf.set({axisX:this.__kj(x),axisY:this.__kj(y),edge:z,align:v});return this.__kf.compute(D,w,t,u);}
,__kg:null,__kh:null,__ki:null,__kj:function(E){switch(E){case o:this.__kg=this.__kg||qx.util.placement.DirectAxis;return this.__kg;case h:this.__kh=this.__kh||qx.util.placement.KeepAlignAxis;return this.__kh;case c:this.__ki=this.__ki||qx.util.placement.BestFitAxis;return this.__ki;default:throw new Error(k);};}
},members:{__ke:null,compute:function(K,H,F,G){{}
;var I=this.getAxisX()||this.__ke;var L=I.computeStart(K.width,{start:F.left,end:F.right},{start:G.left,end:G.right},H.width,this.__kk());var J=this.getAxisY()||this.__ke;var top=J.computeStart(K.height,{start:F.top,end:F.bottom},{start:G.top,end:G.bottom},H.height,this.__kl());return {left:L,top:top};}
,__kk:function(){var N=this.getEdge();var M=this.getAlign();if(N==q){return m;}
else if(N==r){return s;}
else if(M==q){return b;}
else if(M==l){return j;}
else if(M==r){return i;}
;}
,__kl:function(){var P=this.getEdge();var O=this.getAlign();if(P==p){return m;}
...
(function(){var a="Image could not be loaded: ",b="Boolean",c="px",d=".png",e="background-image",f="engine.version",g="scale",h="changeSource",j="x",l="div",m="aborted",n="background-size",o="nonScaled",p="3",q="qx.ui.basic.Image",r="top",s="0 0",t=", no-repeat",u="loaded",v="backgroundImage",w="backgroundRepeat",x="-disabled.$1",y="class",z="qx.event.type.Event",A="image",B="loadingFailed",C="css.alphaimageloaderneeded",D="1.5",E="String",F="browser.documentmode",G="backgroundPosition",H="border-box",I="left",J="_applySource",K="$$widget",L="@",M="px, ",N='.',O="scaled",P=", ",Q="2",R="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7",S="mshtml",T="engine.name",U=", 0 0",V="_applyScale",W="position",X="replacement",Y="img",bl="no-repeat",bm="background-position",bn="hidden",bh="alphaScaled",bi=",",bj="__km",bk="absolute";qx.Class.define(q,{extend:qx.ui.core.Widget,construct:function(bo){this.__km={};qx.ui.core.Widget.call(this);if(bo){this.setSource(bo);}
;}
,properties:{source:{check:E,init:null,nullable:true,event:h,apply:J,themeable:true},scale:{check:b,init:false,themeable:true,apply:V},appearance:{refine:true,init:A},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:z,loaded:z,aborted:z},statics:{PLACEHOLDER_IMAGE:R},members:{__kn:null,__ko:null,__iP:null,__km:null,__kp:null,__kq:null,__kr:0,_onChangeTheme:function(){qx.ui.core.Widget.prototype._onChangeTheme.call(this);this._styleSource();}
...
,_applyDecorator:function(br,bq){qx.ui.core.Widget.prototype._applyDecorator.call(this,br,bq);var bs=this.getSource();bs=qx.util.AliasManager.getInstance().resolve(bs);var bp=this.getContentElement();if(this.__kq){bp=bp.getChild(0);}
;this.__kE(bp,bs);}
...
;}
,renderLayout:function(by,top,bw,bz){qx.ui.core.Widget.prototype.renderLayout.call(this,by,top,bw,bz);var bx=this.getContentElement();if(this.__kq){bx.getChild(0).setStyles({width:bw-(this.getPaddingLeft()||0)-(this.getPaddingRight()||0),height:bz-(this.getPaddingTop()||0)-(this.getPaddingBottom()||0),top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
;}
...
,__ks:function(bF){this.__iP=bF;}
,__kt:function(){if(this.__iP==null){var bH=this.getSource();var bG=false;if(bH!=null){bG=qx.lang.String.endsWith(bH,d);}
;if(this.getScale()&&bG&&qx.core.Environment.get(C)){this.__iP=bh;}
else if(this.getScale()){this.__iP=O;}
else {this.__iP=o;}
;}
;return this.__iP;}
,__ku:function(bK){var bJ;var bI;if(bK==bh){bJ=true;bI=l;}
else if(bK==o){bJ=false;bI=l;}
else {bJ=true;bI=Y;}
;var bM=new qx.html.Image(bI);bM.setAttribute(K,this.toHashCode());bM.setScale(bJ);bM.setStyles({"overflowX":bn,"overflowY":bn,"boxSizing":H});if(qx.core.Environment.get(C)){var bL=this.__kq=new qx.html.Element(l);bL.setAttribute(K,this.toHashCode());bL.setStyle(W,bk);bL.add(bM);return bL;}
;return bM;}
,__kv:function(){if(this.$$disposed){return null;}
;var bN=this.__kt();if(this.__km[bN]==null){this.__km[bN]=this.__ku(bN);}
;var bO=this.__km[bN];if(!this.__kp){this.__kp=bO;}
;return bO;}
,_styleSource:function(){var bT=qx.util.AliasManager.getInstance();var bS=qx.util.ResourceManager.getInstance();var bV=bT.resolve(this.getSource());var bU=this.getContentElement();if(this.__kq){bU=bU.getChild(0);}
;if(!bV){bU.resetSource();return;}
;this.__ky(bV);if((qx.core.Environment.get(T)==S)&&(parseInt(qx.core.Environment.get(f),10)<9||qx.core.Environment.get(F)<9)){var bQ=this.getScale()?g:bl;bU.tagNameHint=qx.bom.element.Decoration.getTagName(bQ,bV);}
;var bR=this.__kx();if(qx.util.ResourceManager.getInstance().has(bV)){var bW=this._findHighResolutionSource(bV);if(bW){var bY=bS.getImageHeight(bV);var bX=bS.getImageWidth(bV);this.setWidth(bY);this.setHeight(bX);var bP=bY+M+bX+c;this.__kp.setStyle(n,bP);this.setSource(bW);bV=bW;}
;this.__kA(bR,bV);this.__kw();}
else if(qx.io.ImageLoader.isLoaded(bV)){this.__kB(bR,bV);this.__kw();}
else {this.__kC(bR,bV);}
;}
,__kw:function(){this.__kr++ ;qx.bom.AnimationFrame.request(function(ca){if(ca===this.__kr){this.fireEvent(u);}
else {this.fireEvent(m);}
;}
.bind(this,this.__kr));}
,__kx:function(){var cb=this.__kp;if(this.__kq){cb=cb.getChild(0);}
;return cb;}
,__ky:qx.core.Environment.select(T,{"mshtml":function(cd){var ce=qx.core.Environment.get(C);var cc=qx.lang.String.endsWith(cd,d);if(ce&&cc){if(this.getScale()&&this.__kt()!=bh){this.__ks(bh);}
else if(!this.getScale()&&this.__kt()!=o){this.__ks(o);}
;}
else {if(this.getScale()&&this.__kt()!=O){this.__ks(O);}
else if(!this.getScale()&&this.__kt()!=o){this.__ks(o);}
;}
;this.__kz(this.__kv());}
,"default":function(cf){if(this.getScale()&&this.__kt()!=O){this.__ks(O);}
else if(!this.getScale()&&this.__kt(o)){this.__ks(o);}
;this.__kz(this.__kv());}
}),__kz:function(cj){var ci=this.__kp;if(ci!=cj){if(ci!=null){var cu=c;var cg={};var co=this.getBounds();if(co!=null){cg.width=co.width+cu;cg.height=co.height+cu;}
;var cp=this.getInsets();cg.left=parseInt(ci.getStyle(I)||cp.left)+cu;cg.top=parseInt(ci.getStyle(r)||cp.top)+cu;cg.zIndex=10;var cm=this.__kq?cj.getChild(0):cj;cm.setStyles(cg,true);cm.setSelectable(this.getSelectable());if(!ci.isVisible()){cj.hide();}
;if(!ci.isIncluded()){cj.exclude();}
;var cr=ci.getParent();if(cr){var ch=cr.getChildren().indexOf(ci);cr.removeAt(ch);cr.addAt(cj,ch);}
;var cl=cm.getNodeName();cm.setSource(null);var ck=this.__kx();cm.tagNameHint=cl;cm.setAttribute(y,ck.getAttribute(y));qx.html.Element.flush();var ct=ck.getDomElement();var cs=cj.getDomElement();var cq=ci.getListeners()||[];cq.forEach(function(cv){cj.addListener(cv.type,cv.handler,cv.self,cv.capture);}
);if(ct&&cs){var cn=ct.$$hash;ct.$$hash=cs.$$hash;cs.$$hash=cn;}
;this.__kp=cj;}
;}
;}
,__kA:function(cx,cz){var cy=qx.util.ResourceManager.getInstance();if(!this.getEnabled()){var cw=cz.replace(/\.([a-z]+)$/,x);if(cy.has(cw)){cz=cw;this.addState(X);}
else {this.removeState(X);}
;}
;if(cx.getSource()===cz){return;}
;this.__kE(cx,cz);this.__kG(cy.getImageWidth(cz),cy.getImageHeight(cz));}
,__kB:function(cA,cE){var cC=qx.io.ImageLoader;this.__kE(cA,cE);var cD=cC.getWidth(cE);var cB=cC.getHeight(cE);this.__kG(cD,cB);}
,__kC:function(cF,cI){var cJ=qx.io.ImageLoader;{var cH,cG,self;}
;if(!cJ.isFailed(cI)){cJ.load(cI,this.__kF,this);}
else {if(cF!=null){cF.resetSource();}
;}
;}
,__kE:function(cK,cO){if(cK.getNodeName()==l){var cS=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());if(cS){var cP=(cS.getStartColor()&&cS.getEndColor());var cN=cS.getBackgroundImage();if(cP||cN){var cL=this.getScale()?g:bl;var cM=qx.bom.element.Decoration.getAttributes(cO,cL);var cR=cS.getStyles(true);var cQ={"backgroundImage":cM.style.backgroundImage,"backgroundPosition":(cM.style.backgroundPosition||s),"backgroundRepeat":(cM.style.backgroundRepeat||bl)};if(cN){cQ[G]+=bi+cR[bm]||s;cQ[w]+=P+cS.getBackgroundRepeat();}
;if(cP){cQ[G]+=U;cQ[w]+=t;}
;cQ[v]+=bi+cR[e];cK.setStyles(cQ);return;}
;}
else {cK.setSource(null);}
;}
;cK.setSource(cO);}
,_findHighResolutionSource:function(cT){var cW=[p,Q,D];var cV=parseFloat(qx.bom.client.Device.getDevicePixelRatio().toFixed(2));if(cV<=1){return false;}
...
;}
;return null;}
,_getHighResolutionSource:function(cX,cY){var db=cX.lastIndexOf(N);if(db>-1){var da=L+cY+j;var dc=cX.slice(0,db)+da+cX.slice(db);if(qx.util.ResourceManager.getInstance().has(dc)){return dc;}
;}
;return null;}
,__kF:function(dd,de){if(this.$$disposed===true){return;}
...
;this.__kB(this.__kx(),dd);}
,__kG:function(df,dg){if(df!==this.__kn||dg!==this.__ko){this.__kn=df;this.__ko=dg;qx.ui.core.queue.Layout.add(this);}
;}
},destruct:function(){for(var dh in this.__km){if(this.__km.hasOwnProperty(dh)){this.__km[dh].setAttribute(K,null,true);}
;}
;delete this.__kp;if(this.__kq){delete this.__kq;}
;this._disposeMap(bj);}
});}
)();
(function(){var a="load",b="",c="qx.io.ImageLoader",d="html.image.naturaldimensions";qx.Bootstrap.define(c,{statics:{__cP:{},__kH:{width:null,height:null},__kI:/\.(png|gif|jpg|jpeg|bmp)\b/i,__kJ:/^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(e){var f=this.__cP[e];return !!(f&&f.loaded);}
,isFailed:function(g){var h=this.__cP[g];return !!(h&&h.failed);}
,isLoading:function(j){var k=this.__cP[j];return !!(k&&k.loading);}
,getFormat:function(p){var o=this.__cP[p];if(!o||!o.format){var m=this.__kJ.exec(p);if(m!=null){var n=(o&&qx.lang.Type.isNumber(o.width)?o.width:this.__kH.width);var q=(o&&qx.lang.Type.isNumber(o.height)?o.height:this.__kH.height);o={loaded:true,format:m[1],width:n,height:q};}
;}
;return o?o.format:null;}
,getSize:function(r){var s=this.__cP[r];return s?{width:s.width,height:s.height}:this.__kH;}
...
;var x=new Image();var C=qx.lang.Function.listener(this.__kK,this,x,z);x.onload=C;x.onerror=C;x.src=z;B.element=x;}
;}
...
;}
;this.__cP[D]=null;}
,__kK:function(){var H=qx.event.GlobalError.observeMethod(this.__kL);H.apply(this,arguments);}
,__kL:function(event,J,I){var N=this.__cP[I];var K=function(O){return (O&&O.height!==0);}
;if(event.type===a&&K(J)){N.loaded=true;N.width=this.__kM(J);N.height=this.__kN(J);var L=this.__kI.exec(I);if(L!=null){N.format=L[1];}
;}
else {N.failed=true;}
...
;}
,__kM:function(P){return qx.core.Environment.get(d)?P.naturalWidth:P.width;}
,__kN:function(Q){return qx.core.Environment.get(d)?Q.naturalHeight:Q.height;}
,dispose:function(){this.__cP={};}
}});}
)();
(function(){var a="source",b="engine.name",c="",d="mshtml",e="px",f="px ",g="no-repeat",h="backgroundImage",i="scale",j="webkit",k="div",l="qx.html.Image",m="qx/static/blank.gif",n="backgroundPosition";qx.Class.define(l,{extend:qx.html.Element,members:{__kO:null,__kP:null,tagNameHint:null,setPadding:function(o,p){this.__kP=o;this.__kO=p;if(this.getNodeName()==k){this.setStyle(n,o+f+p+e);}
;}
...
;var u=this._getProperty(a);var r=this._getProperty(i);var v=r?i:g;if(u!=null){u=u||null;q.paddingTop=this.__kO;q.paddingLeft=this.__kP;qx.bom.element.Decoration.update(s,u,v,q);}
;}
;}
...
;}
,getTagName:function(bc,bb){if(bb&&qx.core.Environment.get(p)&&this.__kR[bc]&&qx.lang.String.endsWith(bb,d)){return k;}
;return this.__kS[bc];}
...
;var bi;if(qx.core.Environment.get(p)&&this.__kR[be]&&bf===x){var bj=this.__kU(bh);this.__kT(bd,bj.width,bj.height);bi=this.processAlphaFix(bd,be,bh);}
else {delete bd.clip;if(be===i){bi=this.__kV(bd,be,bh);}
else if(be===o||be===M){bi=this.__kW(bd,be,bh);}
else {bi=this.__la(bd,be,bh);}
;}
;return bi;}
,__kT:function(bl,bk,bm){if(bl.width==null&&bk!=null){bl.width=bk+c;}
;if(bl.height==null&&bm!=null){bl.height=bm+c;}
;}
,__kU:function(bn){var bo=qx.util.ResourceManager.getInstance().getImageWidth(bn)||qx.io.ImageLoader.getWidth(bn);var bp=qx.util.ResourceManager.getInstance().getImageHeight(bn)||qx.io.ImageLoader.getHeight(bn);return {width:bo,height:bp};}
...
,__kV:function(bw,bx,bv){var by=qx.util.ResourceManager.getInstance().toUri(bv);var bz=this.__kU(bv);this.__kT(bw,bz.width,bz.height);return {src:by,style:bw};}
,__kW:function(bA,bB,bD){var bC=qx.util.ResourceManager.getInstance();var bG=bC.getCombinedFormat(bD);var bI=this.__kU(bD);var bE;if(bG){var bH=bC.getData(bD);var bF=bH[4];if(bG==F){bE=bC.toDataUri(bD);}
else {bE=bC.toUri(bF);}
;if(bB===o){bA=this.__kX(bA,bH,bI.height);}
else {bA=this.__kY(bA,bH,bI.width);}
...
;}
,__kX:function(bJ,bK,bM){var bL=qx.util.ResourceManager.getInstance().getImageHeight(bK[4]);bJ.clip={top:-bK[6],height:bM};bJ.height=bL+c;if(bJ.top!=null){bJ.top=(parseInt(bJ.top,10)+bK[6])+c;}
else if(bJ.bottom!=null){bJ.bottom=(parseInt(bJ.bottom,10)+bM-bL-bK[6])+c;}
;return bJ;}
,__kY:function(bO,bP,bN){var bQ=qx.util.ResourceManager.getInstance().getImageWidth(bP[4]);bO.clip={left:-bP[5],width:bN};bO.width=bQ+c;if(bO.left!=null){bO.left=(parseInt(bO.left,10)+bP[5])+c;}
else if(bO.right!=null){bO.right=(parseInt(bO.right,10)+bN-bQ-bP[5])+c;}
;return bO;}
,__la:function(bR,bS,bV){var bU=qx.util.ResourceManager.getInstance();var bT=bU.getCombinedFormat(bV);var ce=this.__kU(bV);if(bT&&bS!==b){var cd=bU.getData(bV);var cb=cd[4];if(bT==F){var bX=bU.toDataUri(bV);var bW=0;var bY=0;}
...
;this.__kT(bR,ce.width,ce.height);this.__lb(bR,bV,bS);return {style:bR};}
;}
,__lb:function(cg,cj,ch){var top=null;var cm=null;if(cg.backgroundPosition){var ci=cg.backgroundPosition.split(H);cm=parseInt(ci[0],10);if(isNaN(cm)){cm=ci[0];}
...
;}
,__lc:function(cn){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(cn)&&cn.indexOf(a)==-1){if(!this.__kQ[cn]){qx.log.Logger.debug(l+cn);this.__kQ[cn]=true;}
;}
;}
}});}
)();
...
;return y+m+w;}
,__lg:function(A){var String=qx.lang.String;var B=A.substr(0,50);return String.startsWith(B,l)&&String.contains(B,r);}
...
;if(this.__jF[j.getType()]){n=n&&n.getFocusTarget();if(!n){return;}
;}
...
;if(v){if(this.__jF[j.getType()]){v=v.getFocusTarget();}
;if(v===n){return;}
;}
;}
;var r=j.getCurrentTarget();var t=qx.ui.core.Widget.getWidgetByElement(r);if(!t||t.isAnonymous()){return;}
;if(this.__jF[j.getType()]){t=t.getFocusTarget();}
;var u=j.getType();if(!t||!(t.isEnabled()||this.__jG[u])){return;}
...
(function(){var a="json.test.unit.parse.decorator.decorators.ParseTest",b='string',c='parse.json',d='number',e='{"a":5,"b":[2,8],"c":true,"d":"string"}',f='{ "name": "John", "birth": "2011-07-28T22:00:00.000Z" }';qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__cP:{},__tg:{}},members:{__th:function(g){this.__tg.setComponent(g);return this.__tg.getData();}
,setUp:function(){this.__cP=e;this.__tg=new json.parse.decorator.decorators.Parse();}
,testParse:function(){var h=new json.parse.decorator.components.StringData(this.__cP);var i=this.__th(h);this.assertEquals(String(JSON.parse(this.__cP)),String(i));}
,testParseReviver:function(){function k(n,o){if(typeof o===b){var x=Date.parse(o);if(!isNaN(x)){return new Date(x);}
;}
;return o;}
;var m=f;var j=new json.parse.decorator.components.StringData(m);j.setReviver(k);var l=this.__th(j);this.assertEquals(String(JSON.parse(m,k)),String(l));}
,testParseFile:function(){var p=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var s=p.join([c]);var q=new json.parse.decorator.components.FileData(s.getPath());var r=this.__th(q);this.assertEquals(String(JSON.parse(this.__cP)),String(r));}
,testParseFileReviver:function(){var v=function(z,A){if(typeof A===d){A=2*A;}
;return A;}
;var t=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var y=t.join([c]);var u=new json.parse.decorator.components.FileData(y.getPath());u.setReviver(v);var w=this.__th(u);this.assertEquals(String(JSON.parse(this.__cP,v)),String(w));}
}});}
)();
(function(){var a="json.parse.decorator.Component",b="abstract";qx.Class.define(a,{extend:qx.core.Object,type:b,properties:{_reviver:{}},members:{setReviver:function(c){this._reviver=c;}
,getReviver:function(){return this._reviver;}
,getData:function(){}
}});}
)();
(function(){var a="abstract",b="json.parse.decorator.decorators.Decorator";qx.Class.define(b,{extend:json.parse.decorator.Component,type:a,properties:{_component:{},_data:{}},members:{getOriginalData:function(){return this._component.getData();}
,setComponent:function(c){criax.oop.TypeHinting.tHClass(json.parse.decorator.Component,c);this._component=c;}
,getData:function(){if(this._data===null){this._data=this._transform();}
;return this._data;}
,_transform:function(){}
}});}
)();
(function(){var a='classname',b='The type of parameter object has to be ',c="constructor",d="criax.oop.TypeHinting",e='The parameter object must implement interface ',f="static";qx.Class.define(d,{type:f,statics:{tHClass:function(h,g){if((g[a]!==h.classname)&&!(g instanceof h)){throw new Error(b+h.classname);}
;}
,tHInterface:function(j,i){if(!qx.Class.hasInterface(i[c],j)){throw new Error(e+j.name);}
;}
}});}
)();
(function(){var a="json.parse.decorator.decorators.Parse",b="string";qx.Class.define(a,{extend:json.parse.decorator.decorators.Decorator,properties:{},construct:function(){this._data=null;}
,members:{_transform:function(){this._data=this._component.getData();if((typeof this._data)!==b){}
;var c=this._component.getReviver();return JSON.parse(this._data,c);}
}});}
)();
(function(){var a="json.parse.decorator.components.StringData";qx.Class.define(a,{extend:json.parse.decorator.Component,properties:{__sX:{}},construct:function(b){this.__sX=b;this._reviver=null;}
,members:{getData:function(){return this.__sX;}
}});}
)();
(function(){var a="We can not read the file ",b="json.parse.decorator.components.FileData";qx.Class.define(b,{extend:json.parse.decorator.Component,properties:{__ti:{},__tj:{},__cP:{}},construct:function(c){this.__ti=c;this._reviver=null;this.__tj=new criax.chromeless.lib.File(this.__ti);this.__cP=null;this.__tk();}
,members:{getData:function(){if(this.__cP===null){this.__cP=this.__tj.read();}
;return this.__cP;}
,__tk:function(){if(!this.__tj.isReadable()){throw new Error(a+this.__ti);}
;}
}});}
)();
(function(){var a="file",b="Error ",c="moving",d=" exists",e="coping",f="criax.chromeless.lib.MFileSystem",g="directory",h="abstract",i=": ",j=", ",k="fs";qx.Class.define(f,{extend:qx.core.Object,type:h,properties:{_path:{},_route:{},_module:{},_file:{},_fileSystem:{}},construct:function(l){this._path=l;this._module=criax.chromeless.Require.loadChromeless(a);this._route=new criax.chromeless.lib.Path(l);this._fileSystem=criax.chromeless.Require.loadChromeless(k);this._file=this._module.File(this._path);}
,statics:{existsPath:function(m){return criax.chromeless.Require.loadChromeless(k).exists(m);}
,getType:function(n){var o=criax.chromeless.Require.loadChromeless(a).File(n);if(o.isDirectory()){return g;}
;if(o.isFile()){return a;}
;}
},members:{__tl:function(r,s,v){var s=s||false;var p=this.getBaseName();var u=new criax.chromeless.lib.Path(r);var q=u.join([p]);if(criax.chromeless.lib.MFileSystem.existsPath(q.getPath())){if(s===true){if(this.stat().type===a){var t=new criax.chromeless.lib.File(q.getPath());t.remove();}
else {var w=new criax.chromeless.lib.Folder(q.getPath());w.rmDir(true);}
;}
else {throw new Error(b+v+i+this._path+j+this.stat().type+d);}
;}
;}
,copy:function(B,A,x){var A=A||false;this.__tl(B,A,e);this._fileSystem.copy(this._path,B);if(x){var y=new criax.chromeless.lib.Path(B);var z=y.join([this.getBaseName()]);if(this.stat().type===a){return new criax.chromeless.lib.File(z.getPath());}
else {return new criax.chromeless.lib.Folder(z.getPath());}
;}
;return this;}
,move:function(C,D){var D=D||false;this.__tl(C,D,c);this._fileSystem.move(this._path,C);var E=new criax.chromeless.lib.Path(C);this._path=E.join([this.getBaseName()]).getPath();return this;}
,exists:function(){return this._fileSystem.exists(this._path);}
,stat:function(F){var F=F||this._path;return this._fileSystem.stat(F);}
,getBaseName:function(){return this._route.baseName();}
,getDirName:function(){return new criax.chromeless.lib.Folder(this._route.dirName());}
,getPath:function(){return this._path;}
,pathComponents:function(G){return this._route.split(G);}
,getLastModifiedTime:function(H){var H=H||true;return (H)?new Date(this._file.lastModifiedTime):new Date(this._file.lastModifiedTimeOfLink);}
,getPermissions:function(I){return (I)?this._file.permissions:this._file.permissionsOfLink;}
,isHidden:function(){return this._file.isHidden();}
,isSymlink:function(){return this._file.isSymlink();}
,isWritable:function(){return this._file.isWritable();}
,getElementType:function(){var J=criax.chromeless.Require.loadChromeless(a).File(this._path);if(J.isDirectory()){return g;}
;if(J.isFile()){return a;}
;}
}});}
)();
(function(){var a="criax.chromeless.Require",b="static";qx.Class.define(a,{type:b,statics:{__tm:new Array(),loadChromeless:function(c){return this.__tm[c]||(this.__tm[c]=require(c));}
}});}
)();
(function(){var a="_modules",b="criax.chromeless.Modules",c="abstract";var e=criax.chromeless.Require;var d=qx.core.Environment;qx.Class.define(b,{extend:qx.core.Object,type:c,properties:{_module:{}},members:{loadChromeless:function(f){this._module=criax.chromeless.Require.loadChromeless(f);}
,_generatePath:function(g){return this.__tn(this._getSeparator(),g);}
,__tn:function(k,h){var i=k.charAt(0);var j=h;if(h.charAt(h.length-1)!==i){j=h+i;}
;return j;}
,_getSeparator:function(){return criax.chromeless.Os.getSeparatorPath();}
},destruct:function(){this._disposeObjects(a);}
});}
)();
(function(){var a="\\",b="/",c="win",d="static",e="criax.chromeless.Os";qx.Class.define(e,{type:d,statics:{getSeparatorPath:function(){try{var f=criax.chromeless.lib.Process.getOs();}
catch(g){throw g;}
;if(f===c){return a;}
else {return b;}
;}
...
(function(){var a="process",b="criax.chromeless.lib.Process",c="os.name",d="static";qx.Class.define(b,{type:d,statics:{plataformBasePath:function(){return criax.chromeless.Require.loadChromeless(a).cwd();}
,getOs:function(){var e=criax.chromeless.Require.loadChromeless(a).os;var f=qx.core.Environment.get(c);if((e.substring(0,3))==(f.substring(0,3))){return qx.core.Environment.get(c);}
else {return e.substring(0,3);}
;}
}});}
)();
(function(){var a="path",b="criax.chromeless.lib.Path",c='.',d=".";qx.Class.define(b,{extend:criax.chromeless.Modules,properties:{__ti:{}},construct:function(e){this.loadChromeless(a);this.__ti=e;}
,members:{baseName:function(f){var f=d+f||null;return this._module.basename(this.__ti,f);}
,dirName:function(){return this._module.dirname(this.__ti);}
,extName:function(){var g=this._module.extname(this.__ti);return (g[0]==c)?g.substr(1).toLowerCase():g.toLowerCase();}
,join:function(h){var k=h.length;var j=this.__ti;for(var i=0;i<k;i++ ){j=this._module.join(j,h[i]);}
;return new criax.chromeless.lib.Path(j);}
,normalize:function(){return this._module.normalize(this.__ti);}
,split:function(l){var l=l||false;return this._module.split(this.__ti,l);}
,getPath:function(){return this.__ti;}
}});}
)();
(function(){var a="criax.chromeless.lib.File",b="",c="File exists: ",d="byte",f="kb",g="mb",h="File does not exist: ";qx.Class.define(a,{extend:criax.chromeless.lib.MFileSystem,properties:{},construct:function(i){criax.chromeless.lib.MFileSystem.call(this,i);}
,members:{write:function(content){var content=content||b;try{this._module.write(this._path,content);}
catch(e){throw new Error(c+this._path);}
;return this;}
,remove:function(){try{this._fileSystem.remove(this._path);}
catch(e){}
;return this;}
,getExtName:function(){return this._route.extName();}
,__to:function(k,j){if(j){return (this._file.fileSize/k);}
else {return (this._file.fileSizeOfLink/k);}
;}
,__tp:function(m,l){var m=m.toLowerCase()||d;switch(m){case f:return this.__to(1024,l);break;case g:return this.__to(1048576,l);break;case d:return this.__to(1,l);break;};}
,getSize:function(o,n){var n=n||true;var o=o||d;return this.__tp(o,n);}
,isExecutable:function(){return this._file.isExecutable();}
,isReadable:function(){return this._file.isReadable();}
,isSpecial:function(){return this._file.isSpecial();}
,read:function(){try{return this._module.read(this._path);}
catch(e){throw new Error(h+this._path);}
;}
,getMime:function(){var p=new criax.chromeless.lib.Mime(this.getExtName());return p.guess();}
,rename:function(r){if(!this.exists()){throw new Error(h+this._path);}
;var content=this.read();var t=this.getDirName();var s=new criax.chromeless.lib.Path(t.getPath());var q=s.join([r]);var u=new criax.chromeless.lib.File(q.getPath());u.write(content);this.remove();return u;}
},destruct:function(){this.base(arguments);}
});}
)();
(function(){var a="criax.chromeless.lib.Mime",b="mime";qx.Class.define(a,{extend:criax.chromeless.Modules,properties:{__tq:{},__tr:{}},construct:function(c){this.loadChromeless(b);this.__tq=this._module.defaultMime;this.__tr=c;}
,members:{getDefaultMime:function(){return this.__tq;}
,guess:function(){return this._module.guess(this.__tr);}
}});}
)();
(function(){var a="The directory is not empty: ",b="file",c="directory",d="",e="criax.chromeless.lib.Folder";qx.Class.define(e,{extend:criax.chromeless.lib.MFileSystem,properties:{},construct:function(f){criax.chromeless.lib.MFileSystem.call(this,f);this.mkPath();}
,members:{addSubDir:function(g){var h=this._route.join([g]);this._fileSystem.mkpath(h.getPath());return this;}
,addSubDirs:function(j){for(var i=0;i<j.length;i++ ){this.addSubDir(j[i]);}
;return this;}
,hasSubDir:function(l){var k=this.getSubDirs();for(var i=0;i<k.length;i++ ){if(k[i].getBaseName()==l){return i;}
;}
;return -1;}
,getSubDir:function(m){var n=this.hasSubDir(m);return (n!==-1)?this.getSubDirs()[n]:null;}
,getSubDirs:function(){var o=this.list();var q=[];for(var i=0;i<o.length;i++ ){var p=this._route.join([o[i]]);if(this.stat(p.getPath()).type==c){q.push(new criax.chromeless.lib.Folder(p.getPath()));}
;}
;return q;}
,rmSubDir:function(r,s){var t=this.getSubDir(r);if(t instanceof criax.chromeless.lib.Folder){t.rmDir(s);}
;return this;}
,rmSubDirs:function(v){var u=this.getSubDirs();for(var i=0;i<u.length;i++ ){u[i].rmDir(v);}
;return this;}
,addFile:function(w,content){var content=content||d;var y=this._route.join([w]);var x=new criax.chromeless.lib.File(y.getPath());x.write(content);return this;}
,addFiles:function(z){for(var i=0;i<z.length;i++ ){this.addFile(z[i].name,z[i].content);}
;return this;}
,hasFile:function(B){var A=this.getFiles();for(var i=0;i<A.length;i++ ){if(A[i].getBaseName()==B){return i;}
;}
;return -1;}
,getFile:function(D){var C=this.hasFile(D);return (C!==-1)?this.getFiles()[C]:null;}
,getFiles:function(){var F=this.list();var E=[];for(var i=0;i<F.length;i++ ){var G=this._route.join([F[i]]);if(this.stat(G.getPath()).type==b){E.push(new criax.chromeless.lib.File(G.getPath()));}
;}
;return E;}
,removeFile:function(I){var H=this.getFile(I);if(H instanceof criax.chromeless.lib.File){H.remove();}
;}
,removeFiles:function(){var J=this.getFiles();for(var i=0;i<J.length;i++ ){J[i].remove();}
;return this;}
,getElements:function(){var K=this.list();var L=[];for(var i=0;i<K.length;i++ ){var M=this._route.join([K[i]]);if(this.stat(M.getPath()).type==b){L.push(new criax.chromeless.lib.File(M.getPath()));}
else {L.push(new criax.chromeless.lib.Folder(M.getPath()));}
;}
;return L;}
,emptyDir:function(){this.rmDir(true);return criax.chromeless.lib.Folder(this._path);}
,rmDir:function(N){var N=N||false;try{this._fileSystem.rmdir(this._path,N);}
catch(O){throw new Error(a+this._path);}
;}
,numElements:function(){return this.stat().numEntries;}
,list:function(){return this._fileSystem.list(this._path);}
,mkPath:function(){this._fileSystem.mkpath(this._path);}
},destruct:function(){this.base(arguments);}
});}
)();
(function(){var a="resource/",b="criax.chromeless.lib.AppPath",c="app-paths",d="resource",e="index.html";qx.Class.define(b,{statics:{getAppDir:function(){var f=window.location.href;var g=f.indexOf(e);return f.substring(0,g);}
,getAppResourceDir:function(){return criax.chromeless.lib.AppPath.getAppDir()+a;}
,getBrowserCodeDir:function(){return criax.chromeless.Require.loadChromeless(c).browserCodeDir;}
,getCurDir:function(){return criax.chromeless.Require.loadChromeless(c).curDir;}
,getDesktopDir:function(){return criax.chromeless.Require.loadChromeless(c).desktopDir;}
,getPluginsDir:function(){return criax.chromeless.Require.loadChromeless(c).pluginsDir;}
,getProfileDir:function(){return criax.chromeless.Require.loadChromeless(c).profileDir;}
,getProfileRootDir:function(){return criax.chromeless.Require.loadChromeless(c).profileRootDir;}
,getWindowStartMenuDir:function(){return criax.chromeless.Require.loadChromeless(c).startMenuDir;}
,getTmpDir:function(){return criax.chromeless.Require.loadChromeless(c).tmpDir;}
,getUserHomeDir:function(){return criax.chromeless.Require.loadChromeless(c).userHomeDir;}
,getResourceDir:function(){var h=new criax.chromeless.lib.Path(this.getBrowserCodeDir());var i=h.join([d]);return i.getPath();}
}});}
)();
(function(){var a='string',b="json.test.integration.facade.ExternalDecorator",c='number';qx.Class.define(b,{extend:json.parse.decorator.decorators.Decorator,properties:{},construct:function(){this._data=null;}
,members:{_transform:function(){this._data=this._component.getData();return JSON.parse(this._data,function(d,e){if(typeof e===c){e=0;}
;if(typeof e===a){e=e.toUpperCase();}
;return e;}
);}
}});}
)();
(function(){var a='parse.json',b="json.test.unit.parse.decorator.components.FileDataTest",c='{"a":5,"b":[2,8],"c":true,"d":"string"}';qx.Class.define(b,{extend:qx.dev.unit.TestCase,properties:{__ti:{}},members:{setUp:function(){var d=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var e=d.join([a]);this.__ti=e.getPath();}
,testFile:function(){var f=new json.parse.decorator.components.FileData(this.__ti);this.assertEquals(c,f.getData());}
,testInitialReviver:function(){var g=new json.parse.decorator.components.FileData(this.__ti);this.assertNull(g.getReviver());}
,testReviverFunction:function(){var h=new json.parse.decorator.components.FileData(this.__ti);h.setReviver(function(i){}
);this.assertFunction(h.getReviver());}
,testReviverArray:function(){var j=new json.parse.decorator.components.FileData(this.__ti);j.setReviver([1,2,3]);this.assertArray(j.getReviver());}
}});}
)();
(function(){var a="json.test.unit.stringify.decorator.decorators.ModelToJsTest",b="string",c='number';qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__ts:{},__tt:{},__tu:{}},members:{__th:function(){this.__tu.setComponent(this.__tt);return this.__tu.getData();}
,setUp:function(){this.__ts=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:b});this.__tt=new json.stringify.decorator.components.ModelData(this.__ts);this.__tu=new json.stringify.decorator.decorators.ModelToJs();}
,testModel:function(){var d=this.__th();this.assertEquals(5,d.a);}
,testModelReplacer:function(){var e=function(g,h){if(typeof h===c){h=2*h;}
;return h;}
;this.__tt.setReplacer(e);var f=this.__th();this.assertEquals(10,f.a);}
}});}
)();
...
(function(){var a='"',b="Class '",c="",d="Unsupported type!",e="change",f="qx.data.marshal.Json",g="Array",h="_validate",j="' could not be found.",k="]",l="[",m="set",n="_applyEventPropagation",o="qx.data.model.";qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(p){qx.core.Object.call(this);this.__mT=p;}
...
},members:{__mT:null,__mU:function(s){return Object.keys(s).sort().join(a);}
,toClass:function(t,u){this.__mV(t,u,null,0);}
,__mV:function(G,B,v,D){if(!qx.lang.Type.isObject(G)||!!G.$$isString||G instanceof qx.core.Object){if(G instanceof Array||qx.Bootstrap.getClass(G)==g){for(var i=0;i<G.length;i++ ){this.__mV(G[i],B,v+l+i+k,D+1);}
;}
;return;}
;var x=this.__mU(G);if(this.__na(x,v,D)){return;}
;for(var F in G){this.__mV(G[F],B,F,D+1);}
;if(qx.Class.isDefined(o+x)){return;}
;if(this.__mT&&this.__mT.getModelClass&&this.__mT.getModelClass(x,G,v,D)!=null){return;}
;var H={};var y={__mW:this.__mW};for(var F in G){if(this.__mT&&this.__mT.getPropertyMapping){F=this.__mT.getPropertyMapping(F,x);}
;F=F.replace(/-|\.|\s+/g,c);{}
;H[F]={};H[F].nullable=true;H[F].event=e+qx.lang.String.firstUp(F);if(B){H[F].apply=n;}
;if(this.__mT&&this.__mT.getValidationRule){var A=this.__mT.getValidationRule(x,F);if(A){H[F].validate=h+F;y[h+F]=A;}
;}
;}
;if(this.__mT&&this.__mT.getModelSuperClass){var E=this.__mT.getModelSuperClass(x,v,D)||qx.core.Object;}
else {var E=qx.core.Object;}
;var z=[];if(this.__mT&&this.__mT.getModelMixins){var C=this.__mT.getModelMixins(x,v,D);if(!qx.lang.Type.isArray(C)){if(C!=null){z=[C];}
;}
else {z=C;}
;}
;if(B){z.push(qx.data.marshal.MEventBubbling);}
;var w={extend:E,include:z,properties:H,members:y,destruct:this.__mX};qx.Class.define(o+x,w);}
,__mX:function(){var I=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var J in I){this.__mW(this.get(I[J].name));}
;}
,__mW:function(K){if(!(K instanceof qx.core.Object)){return;}
;if(K.isDisposed()){return;}
;K.dispose();}
,__mY:function(L,P,Q,O){var R;if(this.__mT&&this.__mT.getModelClass){R=this.__mT.getModelClass(L,P,Q,O);}
;if(R!=null){return (new R());}
else {var N=o+L;var M=qx.Class.getByName(N);if(!M){throw new Error(b+N+j);}
;return (new M());}
;}
,__na:function(S,V,T){var U=this.__mT;return U&&U.ignore&&U.ignore(S,V,T);}
,toModel:function(W){return this.__nb(W,null,0);}
,__nb:function(bj,Y,bf){var bd=qx.lang.Type.isObject(bj);var X=bj instanceof Array||qx.Bootstrap.getClass(bj)==g;if((!bd&&!X)||!!bj.$$isString||bj instanceof qx.core.Object){return bj;}
else if(this.__na(this.__mU(bj),Y,bf)){return bj;}
else if(X){var bh=qx.data.Array;if(this.__mT&&this.__mT.getArrayClass){var be=this.__mT.getArrayClass(Y,bf);bh=be||bh;}
;var bk=new bh();bk.setAutoDisposeItems(true);for(var i=0;i<bj.length;i++ ){bk.push(this.__nb(bj[i],Y+l+i+k,bf+1));}
;return bk;}
else if(bd){var ba=this.__mU(bj);var bi=this.__mY(ba,bj,Y,bf);for(var bg in bj){var bb=bg;if(this.__mT&&this.__mT.getPropertyMapping){bb=this.__mT.getPropertyMapping(bg,ba);}
;var bl=bb.replace(/-|\.|\s+/g,c);{}
;bb=bl;var bc=m+qx.lang.String.firstUp(bb);if(bi[bc]){bi[bc](this.__nb(bj[bg],bg,bf+1));}
;}
;return bi;}
;throw new Error(d);}
...
;if((n instanceof qx.core.Object)&&qx.Class.hasMixin(n.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__nc,this,name);var q=n.addListener(g,o,this);var p=n.getUserData(d+this.$$hash);if(p==null){p=[];n.setUserData(d+this.$$hash,p);}
;p.push(q);}
;}
,__nc:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(h)!=-1?y.name.indexOf(h):y.name.length;var v=y.name.indexOf(f)!=-1?y.name.indexOf(f):y.name.length;if(v==0){var t=name+y.name;}
...
;}
else if(typeof r==n){this.__nd=new Array(r);}
else if(r instanceof Array){this.__nd=qx.lang.Array.clone(r);}
else {this.__nd=[];this.dispose();throw new Error(j);}
;for(var i=0;i<this.__nd.length;i++ ){this._applyEventPropagation(this.__nd[i],null,i);}
;this.__ne();{}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":q,"changeLength":q},members:{__nd:null,concat:function(s){if(s){var t=this.__nd.concat(s);}
else {var t=this.__nd.concat();}
;return new qx.data.Array(t);}
,join:function(u){return this.__nd.join(u);}
,pop:function(){var v=this.__nd.pop();this.__ne();this._registerEventChaining(null,v,this.length-1);this.fireDataEvent(o,{value:[],name:this.length+f,old:[v],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:l,removed:[v],added:[]},null);return v;}
,push:function(w){for(var i=0;i<arguments.length;i++ ){this.__nd.push(arguments[i]);this.__ne();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(o,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var x=this.__nd.concat();this.__nd.reverse();this.__nf(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__nd,name:k+(this.__nd.length-1),old:x,item:this});}
,shift:function(){if(this.length==0){return;}
;var y=this.__nd.shift();this.__ne();this._registerEventChaining(null,y,this.length-1);this.__nf(0,this.length);this.fireDataEvent(o,{value:[],name:m,old:[y],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:l,removed:[y],added:[]},null);return y;}
,slice:function(A,z){return new qx.data.Array(this.__nd.slice(A,z));}
,splice:function(F,H,J){var N=this.__nd.length;var I=this.__nd.splice.apply(this.__nd,arguments);if(this.__nd.length!=N){this.__ne();}
...
;this.__nf(F+(arguments.length-2)-H,this.length);if(L||D){var G=[];for(var i=2;i<arguments.length;i++ ){G[i-2]=arguments[i];}
...
;this.__nf(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__nd,name:k+(this.length-1),old:O,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i-- ){this.__nd.unshift(arguments[i]);this.__ne();this.__nf(0,this.length);this.fireDataEvent(o,{value:[this.__nd[0]],name:m,old:[this.__nd[1]],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__nd;}
,getItem:function(R){return this.__nd[R];}
,setItem:function(S,U){var T=this.__nd[S];if(T===U){return;}
;this.__nd[S]=U;this._registerEventChaining(U,T,S);if(this.length!=this.__nd.length){this.__ne();}
...
,indexOf:function(V){return this.__nd.indexOf(V);}
,lastIndexOf:function(W){return this.__nd.lastIndexOf(W);}
,toString:function(){if(this.__nd!=null){return this.__nd.toString();}
;return f;}
,contains:function(X){return this.__nd.indexOf(X)!==-1;}
...
;var bl=this.getLength();var bk=this.__nd.concat();this.__nd.length=0;this.__ne();this.fireDataEvent(o,{value:[],name:k+(bl-1),old:bk,item:this});this.fireDataEvent(g,{start:0,end:bl-1,type:l,removed:bk,added:[]},null);return bk;}
,append:function(bm){if(bm instanceof qx.data.Array){bm=bm.toArray();}
;{}
;var bn=this.__nd.length;Array.prototype.push.apply(this.__nd,bm);for(var i=0;i<bm.length;i++ ){this._registerEventChaining(bm[i],null,bn+i);}
;var bn=this.length;this.__ne();var name=bn==(this.length-1)?bn:bn+a+(this.length-1);this.fireDataEvent(o,{value:bm,name:name+f,old:[],item:this});this.fireDataEvent(g,{start:bn,end:this.length-1,type:b,added:bm,removed:[]},null);}
...
,forEach:function(bu,bv){for(var i=0;i<this.__nd.length;i++ ){bu.call(bv,this.__nd[i],i,this);}
;}
,filter:function(bw,self){return new qx.data.Array(this.__nd.filter(bw,self));}
,map:function(bx,self){return new qx.data.Array(this.__nd.map(bx,self));}
,some:function(by,self){return this.__nd.some(by,self);}
,every:function(bz,self){return this.__nd.every(bz,self);}
,reduce:function(bB,bA){return this.__nd.reduce(bB,bA);}
,reduceRight:function(bD,bC){return this.__nd.reduceRight(bD,bC);}
,__ne:function(){var bE=this.length;this.length=this.__nd.length;this.fireDataEvent(p,this.length,bE);}
,__nf:function(bG,bF){for(var i=bG;i<bF;i++ ){this._registerEventChaining(this.__nd[i],this.__nd[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__nd.length;i++ ){var bH=this.__nd[i];this._applyEventPropagation(null,bH,i);if(this.isAutoDisposeItems()&&bH&&bH instanceof qx.core.Object){bH.dispose();}
;}
;this.__nd=null;}
});}
)();
(function(){var a="abstract",b="json.stringify.decorator.Component";qx.Class.define(b,{extend:qx.core.Object,type:a,properties:{_replacer:{},_space:{}},members:{setReplacer:function(c){this._replacer=c;}
,getReplacer:function(){return this._replacer;}
,setSpace:function(d){this._space=d;}
,getSpace:function(){return this._space;}
,getData:function(){}
}});}
)();
(function(){var a="json.stringify.decorator.components.ModelData";qx.Class.define(a,{extend:json.stringify.decorator.Component,properties:{__sX:{}},construct:function(b){this.__sX=b;this._replacer=null;this._space=0;}
,members:{getData:function(){return this.__sX;}
}});}
)();
(function(){var a="abstract",b="json.stringify.decorator.decorators.Decorator";qx.Class.define(b,{extend:json.stringify.decorator.Component,type:a,properties:{_component:{},_data:{}},members:{getOriginalData:function(){return this._component.getData();}
,setComponent:function(c){criax.oop.TypeHinting.tHClass(json.stringify.decorator.Component,c);this._component=c;}
,getData:function(){if(this._data===null){this._data=this._transform();}
;return this._data;}
,_transform:function(){}
}});}
)();
(function(){var a="json.stringify.decorator.decorators.ModelToJs",b="$$user_",c='$$user_autoDisposeItems';qx.Class.define(a,{extend:json.stringify.decorator.decorators.Decorator,properties:{},construct:function(){this._data=null;}
,members:{_transform:function(){this._data=this._component.getData();var d=this.__tv(this._data);var f=this._component.getReplacer();var g=this._component.getSpace();var e=JSON.stringify(d,f,g);return JSON.parse(e);}
,__tv:function(k){var j={};for(var l in k){var h=l.indexOf(b);if(qx.lang.Type.isObject(this._data[l])){if(h!==-1){j[l.substring(7)]=this.__tv(k[l]);}
;}
else {if(qx.lang.Type.isArray(k[l])){return this.__tw(k[l]);}
else {if(h!==-1){if(l!==c){j[l.substring(7)]=k[l];}
;}
;}
;}
;}
;return j;}
,__tw:function(m){var n=[];for(var i=0;i<m.length;i++ ){if(qx.lang.Type.isObject(m[i])){n.push(this._transform(m[i]));}
else if(qx.lang.Type.isArray(m[i])){n.push(this.__tw(m[i]));}
else {n.push(m[i]);}
;}
;return n;}
}});}
)();
(function(){var a="json.test.integration.parse.builder.directors.DirectorTest",b='parse.json',c='{"a":10,"b":[4,16],"c":true,"d":"string"}',d='number';qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__tx:{},__cP:{}},members:{__th:function(g){var e=new json.parse.builder.directors.Director();e.setBuilder(g);e.buildComponent();var f=g.getComponent();return f.getData();}
,setUp:function(){this.__tx=function(h,i){if(typeof i===d){i=2*i;}
;return i;}
;this.__cP=c;}
,testFileToParse:function(){var j=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var m=j.join([b]);var k=new json.parse.builder.builders.FileToParse(m.getPath(),this.__tx);var l=this.__th(k);this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(l));}
,testFileToModel:function(){var n=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var q=n.join([b]);var o=new json.parse.builder.builders.FileToModel(q.getPath(),this.__tx);var p=this.__th(o);this.assertEquals(10,p.getA());}
,testStringToParse:function(){var s=new json.parse.builder.builders.StringToParse(this.__cP,this.__tx);var r=this.__th(s);this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(r));}
,testStringToModel:function(){var u=new json.parse.builder.builders.StringToModel(this.__cP,this.__tx);var t=this.__th(u);this.assertEquals(20,t.getA());}
}});}
)();
(function(){var a="json.parse.builder.directors.Director";qx.Class.define(a,{extend:qx.core.Object,properties:{__ty:{}},construct:function(){}
,members:{setBuilder:function(b){criax.oop.TypeHinting.tHClass(json.parse.builder.builders.Builder,b);this.__ty=b;}
,buildComponent:function(){this.__ty.createComponent();this.__ty.buildReviver();this.__ty.buildDecorator();}
}});}
)();
(function(){var a="json.parse.builder.builders.Builder",b="abstract";qx.Class.define(a,{extend:qx.core.Object,type:b,properties:{_from:{},_to:{},_reviver:{}},members:{getComponent:function(){return this._to;}
,createComponent:function(){}
,buildReviver:function(){}
,buildDecorator:function(){}
}});}
)();
(function(){var a="json.parse.builder.builders.FileToParse";qx.Class.define(a,{extend:json.parse.builder.builders.Builder,properties:{__ti:{}},construct:function(b,c){this.__ti=b;this._reviver=c||null;}
,members:{createComponent:function(){this._from=new json.parse.decorator.components.FileData(this.__ti);}
,buildReviver:function(){this._from.setReviver(this._reviver);}
,buildDecorator:function(){this._to=new json.parse.decorator.decorators.Parse();this._to.setComponent(this._from);}
}});}
)();
(function(){var a="json.parse.builder.builders.FileToModel";qx.Class.define(a,{extend:json.parse.builder.builders.Builder,properties:{__ti:{}},construct:function(b,c){this.__ti=b;this._reviver=c;}
,members:{createComponent:function(){this._from=new json.parse.decorator.components.FileData(this.__ti);}
,buildReviver:function(){this._from.setReviver(this._reviver);}
,buildDecorator:function(){var d=new json.parse.decorator.decorators.Parse();d.setComponent(this._from);this._to=new json.parse.decorator.decorators.Model();this._to.setComponent(d);}
}});}
)();
(function(){var a="json.parse.decorator.decorators.Model",b="string";qx.Class.define(a,{extend:json.parse.decorator.decorators.Decorator,properties:{},construct:function(){this._data=null;}
,members:{_transform:function(){this._data=this._component.getData();if((typeof this._data)===b){var c=new json.parse.decorator.decorators.Parse();c.setComponent(this._component);this._data=c.getData();this.setComponent(c);}
else {var d=JSON.stringify(this._data);var e=this._component.getReviver();this._data=JSON.parse(d,e);}
;return qx.data.marshal.Json.createModel(this._data);}
}});}
)();
(function(){var a="json.parse.builder.builders.StringToParse";qx.Class.define(a,{extend:json.parse.builder.builders.Builder,properties:{__sX:{}},construct:function(c,b){this.__sX=c;this._reviver=b;}
,members:{createComponent:function(){this._from=new json.parse.decorator.components.StringData(this.__sX);}
,buildReviver:function(){this._from.setReviver(this._reviver);}
,buildDecorator:function(){this._to=new json.parse.decorator.decorators.Parse();this._to.setComponent(this._from);}
}});}
)();
(function(){var a="json.parse.builder.builders.StringToModel";qx.Class.define(a,{extend:json.parse.builder.builders.Builder,properties:{__sX:{}},construct:function(c,b){this.__sX=c;this._reviver=b;}
,members:{createComponent:function(){this._from=new json.parse.decorator.components.StringData(this.__sX);}
,buildReviver:function(){this._from.setReviver(this._reviver);}
,buildDecorator:function(){var d=new json.parse.decorator.decorators.Parse();d.setComponent(this._from);this._to=new json.parse.decorator.decorators.Model();this._to.setComponent(d);}
}});}
)();
(function(){var a="string",b='parse_new.json',c="json.test.integration.stringify.builder.directors.DirectorTest",d='{"a":10,"b":[4,16],"c":true,"d":"string"}',e='number';qx.Class.define(c,{extend:qx.dev.unit.TestCase,properties:{__tz:{},__tA:{},__cP:{}},members:{__th:function(h){var f=new json.stringify.builder.directors.Director();f.setBuilder(h);f.buildComponent();var g=h.getComponent();return g.getData();}
,setUp:function(){this.__tz=function(i,j){if(typeof j===e){j=2*j;}
;return j;}
;this.__tA=0;this.__cP=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:a});}
,testModelToJs:function(){var k={a:10,b:[4,16],c:true,d:a};var m=new json.stringify.builder.builders.ModelToJs(this.__cP,this.__tz,this.__tA);var l=this.__th(m);this.assertEquals(JSON.stringify(k),JSON.stringify(l));}
,testModelToStringify:function(){var o={a:10,b:[4,16],c:true,d:a};var n=new json.stringify.builder.builders.ModelToStringify(this.__cP,this.__tz,this.__tA);var p=this.__th(n);this.assertEquals(JSON.stringify(o),p);}
,testModelToFile:function(){var q=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var u=q.join([b]);var s=d;var r=new json.stringify.builder.builders.ModelToFile(this.__cP,this.__tz,this.__tA);r.setPath(u.getPath());var t=this.__th(r);this.assertEquals(s,t);}
}});}
)();
(function(){var a="json.stringify.builder.directors.Director";qx.Class.define(a,{extend:qx.core.Object,properties:{__ty:{}},construct:function(){}
,members:{setBuilder:function(b){criax.oop.TypeHinting.tHClass(json.stringify.builder.builders.Builder,b);this.__ty=b;}
,buildComponent:function(){this.__ty.createComponent();this.__ty.buildReplacer();this.__ty.buildSpace();this.__ty.buildDecorator();}
}});}
)();
(function(){var a="json.stringify.builder.builders.Builder",b="abstract";qx.Class.define(a,{extend:qx.core.Object,type:b,properties:{_from:{},_to:{},_replacer:{},_space:{},_modelQx:{}},members:{getComponent:function(){return this._to;}
,createComponent:function(){}
...
(function(){var a="json.stringify.builder.builders.ModelToJs";qx.Class.define(a,{extend:json.stringify.builder.builders.Builder,properties:{},construct:function(c,b,d){this._modelQx=c;this._replacer=b||null;this._space=d||0;}
,members:{createComponent:function(){this._from=new json.stringify.decorator.components.ModelData(this._modelQx);}
,buildReplacer:function(){this._from.setReplacer(this._replacer);}
,buildSpace:function(){this._from.setSpace(this._space);}
,buildDecorator:function(){this._to=new json.stringify.decorator.decorators.ModelToJs();this._to.setComponent(this._from);}
}});}
)();
(function(){var a="json.stringify.builder.builders.ModelToStringify";qx.Class.define(a,{extend:json.stringify.builder.builders.Builder,properties:{},construct:function(c,b,d){this._modelQx=c;this._replacer=b||null;this._space=d||0;}
,members:{createComponent:function(){this._from=new json.stringify.decorator.components.ModelData(this._modelQx);}
,buildReplacer:function(){this._from.setReplacer(this._replacer);}
,buildSpace:function(){this._from.setSpace(this._space);}
,buildDecorator:function(){var e=new json.stringify.decorator.decorators.ModelToJs();e.setComponent(this._from);this._to=new json.stringify.decorator.decorators.Stringify();this._to.setComponent(e);}
}});}
)();
(function(){var a="json.stringify.decorator.decorators.Stringify";qx.Class.define(a,{extend:json.stringify.decorator.decorators.Decorator,properties:{},construct:function(){this._data=null;}
,members:{_transform:function(){this._data=this._component.getData();var b=this._component.getReplacer();var c=this._component.getSpace();return JSON.stringify(this._data,b,c);}
}});}
)();
(function(){var a="json.stringify.builder.builders.ModelToFile";qx.Class.define(a,{extend:json.stringify.builder.builders.Builder,properties:{__ti:{}},construct:function(c,b,d){this._modelQx=c;this._replacer=b||null;this._space=d||0;}
,members:{createComponent:function(){this._from=new json.stringify.decorator.components.ModelData(this._modelQx);}
,buildReplacer:function(){this._from.setReplacer(this._replacer);}
,buildSpace:function(){this._from.setSpace(this._space);}
,buildDecorator:function(){var e=new json.stringify.decorator.decorators.ModelToJs();e.setComponent(this._from);var f=new json.stringify.decorator.decorators.Stringify();f.setComponent(e);this._to=new json.stringify.decorator.decorators.WriteFile(this.__ti);this._to.setComponent(f);}
,setPath:function(g){this.__ti=g;}
}});}
)();
(function(){var a="json.stringify.decorator.decorators.WriteFile";qx.Class.define(a,{extend:json.stringify.decorator.decorators.Decorator,properties:{__ti:{},__tj:{}},construct:function(b){this.__ti=b;this.__tj=new criax.chromeless.lib.File(this.__ti);this._data=null;}
,members:{_transform:function(){var g=this._component.getData();var f=JSON.parse(g);var c=this._component.getReplacer();var d=this._component.getSpace();var e=JSON.stringify(f,c,d);var h=this.__tB(e);this._data=h.read();return this._data;}
,__tB:function(i){if(this.__tj.exists()){this.__tj.remove();}
;return this.__tj.write(i);}
}});}
)();
(function(){var a="STRING",b='parse.json',c="json.test.integration.facade.ParseTest",d='{"a":10,"b":[4,16],"c":true,"d":"string"}',e='number';qx.Class.define(c,{extend:qx.dev.unit.TestCase,properties:{__tx:{},__cP:{}},members:{setUp:function(){this.__tx=function(f,g){if(typeof g===e){g=2*g;}
;return g;}
;this.__cP=d;}
,testFileToModel:function(){var i=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var k=i.join([b]);var h=new json.facade.Parse();h.setValue(k.getPath());h.setReviver(this.__tx);h.setModel(true);h.setFile(true);var j=h.parse();this.assertEquals(10,j.getA());}
,testFileToParse:function(){var m=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var o=m.join([b]);var l=new json.facade.Parse();l.setValue(o.getPath());l.setReviver(this.__tx);l.setFile(true);var n=l.parse();this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(n));}
,testStringToParse:function(){var p=new json.facade.Parse();p.setValue(this.__cP);p.setReviver(this.__tx);var q=p.parse();this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(q));}
,testStringToModel:function(){var r=new json.facade.Parse();r.setValue(this.__cP);r.setReviver(this.__tx);r.setModel(true);var s=r.parse();this.assertEquals(20,s.getA());}
,testExternalBuilder:function(){var t=new json.facade.Parse();t.setValue(this.__cP);t.setBuilder(new json.test.integration.facade.ExternalBuilder(this.__cP));var u=t.parse();this.assertEquals(a,u.d);}
}});}
)();
(function(){var a="json.facade.Transform",b="abstract";qx.Class.define(a,{extend:qx.core.Object,type:b,properties:{_value:{},_file:{},_builder:{}},members:{setValue:function(c){this._value=c;return this;}
,setFile:function(d){this._file=d;return this;}
,setBuilder:function(e){this._builder=e;return this;}
,_transform:function(f){if(this._builder===null){this._builder=this._getComponentBuilder();}
;f.setBuilder(this._builder);f.buildComponent();var g=this._builder.getComponent();return g.getData();}
,_getComponentBuilder:function(){}
}});}
)();
(function(){var a="json.facade.Parse",b='There is not a builder defined',c="";qx.Class.define(a,{extend:json.facade.Transform,properties:{__tx:{},__tC:{}},construct:function(){this.__sX=c;this.__tx=null;this.__tC=false;this._builder=null;this._file=false;}
,members:{_getComponentBuilder:function(){if(this._file&&this.__tC){return new json.parse.builder.builders.FileToModel(this._value,this.__tx);}
;if(!this._file&&!this.__tC){return new json.parse.builder.builders.StringToParse(this._value,this.__tx);}
;if(this._file&&!this.__tC){return new json.parse.builder.builders.FileToParse(this._value,this.__tx);}
;if(!this._file&&this.__tC){return new json.parse.builder.builders.StringToModel(this._value,this.__tx);}
;throw new Error(b);}
,parse:function(){return this._transform(new json.parse.builder.directors.Director());}
,setReviver:function(d){this.__tx=d;return this;}
,setModel:function(e){this.__tC=e;return this;}
}});}
)();
(function(){var a="json.test.integration.facade.ExternalBuilder";qx.Class.define(a,{extend:json.parse.builder.builders.Builder,properties:{__sX:{}},construct:function(b){this.__sX=b;}
,members:{buildDecorator:function(){this._to=new json.test.integration.facade.ExternalDecorator();this._to.setComponent(new json.parse.decorator.components.StringData(this.__sX));}
}});}
)();
(function(){var a="json.test.unit.parse.decorator.decorators.ModelTest",b='number',c='parse.json',d='{"a":5,"b":[2,8],"c":true,"d":"string"}';qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__ti:{},__cP:{},__tC:{},__tg:{},__tx:{}},members:{__th:function(e){this.__tg.setComponent(e);this.__tC.setComponent(this.__tg);return this.__tC.getData();}
,setUp:function(){this.__cP=d;this.__tg=new json.parse.decorator.decorators.Parse();this.__tC=new json.parse.decorator.decorators.Model();this.__tx=function(f,g){if(typeof g===b){g=2*g;}
;return g;}
;}
,testModel:function(){var h=new json.parse.decorator.components.StringData(this.__cP);var i=this.__th(h);this.assertEquals(5,i.getA());}
,testModelReviver:function(){var j=new json.parse.decorator.components.StringData(this.__cP);j.setReviver(this.__tx);var k=this.__th(j);this.assertEquals(16,k.getB().getItem(1));}
,testModelString:function(){var l=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var o=l.join([c]);var m=new json.parse.decorator.components.FileData(o.getPath());m.setReviver(this.__tx);this.__tC.setComponent(m);var n=this.__tC.getData();this.assertEquals(10,n.getA());}
}});}
)();
(function(){var a="json.test.unit.stringify.decorator.components.ModelDataTest",b="string",c='number';qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__tC:{},__tt:{}},members:{setUp:function(){this.__tC=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:b});this.__tt=new json.stringify.decorator.components.ModelData(this.__tC);}
,testValue:function(){var d=this.__tt.getData();this.assertEquals(this.__tC.getA(),d.getA());}
,testInitialReplacer:function(){this.assertNull(this.__tt.getReplacer());}
,testReplacerFunction:function(){var e=function(f,g){if(typeof g===c){g=2*g;}
;return g;}
;this.__tt.setReplacer(e);this.assertFunction(this.__tt.getReplacer());}
}});}
)();
(function(){var a='parse_new.json',b="json.test.integration.facade.StringifyTest",c="string",d='{"a":10,"b":[4,16],"c":true,"d":"string"}',e='number';qx.Class.define(b,{extend:qx.dev.unit.TestCase,properties:{__tz:{},__tA:{},__cP:{}},members:{setUp:function(){this.__tz=function(f,g){if(typeof g===e){g=2*g;}
;return g;}
;this.__tA=0;this.__cP=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:c});}
,testModelToJs:function(){var i={a:10,b:[4,16],c:true,d:c};var h=new json.facade.Stringify();h.setValue(this.__cP);h.setReplacer(this.__tz);h.setSpace(this.__tA);h.setJsObject(true);var j=h.stringify();this.assertEquals(JSON.stringify(i),JSON.stringify(j));}
,testModelToStringify:function(){var l={a:10,b:[4,16],c:true,d:c};var k=new json.facade.Stringify();k.setValue(this.__cP);k.setReplacer(this.__tz);k.setSpace(this.__tA);var m=k.stringify();this.assertEquals(JSON.stringify(l),m);}
,testModelToFile:function(){var n=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var r=n.join([a]);var p=d;var o=new json.facade.Stringify();o.setValue(this.__cP);o.setReplacer(this.__tz);o.setSpace(this.__tA);o.setFile(true);o.setPath(r.getPath());var q=o.stringify();this.assertEquals(p,q);}
}});}
)();
(function(){var a='There is not a builder defined',b="json.facade.Stringify",c="";qx.Class.define(b,{extend:json.facade.Transform,properties:{__tz:{},__tA:{},__ti:{},__tD:{}},construct:function(){this.__sX={};this.__tz=null;this.__tA=0;this.__ti=c;this.__tD=false;this._file=false;this._builder=null;}
,members:{_getComponentBuilder:function(){if(this._file&&(this.__ti.length>0)){var d=new json.stringify.builder.builders.ModelToFile(this._value,this.__tz,this.__tA);d.setPath(this.__ti);return d;}
;if(!this._file&&this.__tD){return new json.stringify.builder.builders.ModelToJs(this._value,this.__tz,this.__tA);}
;if(!this._file&&!this.__tD){return new json.stringify.builder.builders.ModelToStringify(this._value,this.__tz,this.__tA);}
;throw new Error(a);}
,stringify:function(){return this._transform(new json.stringify.builder.directors.Director());}
,setReplacer:function(e){this.__tz=e;return this;}
,setSpace:function(f){this.__tA=f;return this;}
,setPath:function(g){this.__ti=g;return this;}
,setJsObject:function(h){this.__tD=h;return this;}
}});}
)();
(function(){var a="json.test.unit.parse.decorator.components.StringDataTest",b='value';qx.Class.define(a,{extend:qx.dev.unit.TestCase,members:{testValue:function(){var c=new json.parse.decorator.components.StringData(b);this.assertEquals(b,c.getData());}
,testInitialReviver:function(){var d=new json.parse.decorator.components.StringData(b);this.assertNull(d.getReviver());}
,testReviverFunction:function(){var e=new json.parse.decorator.components.StringData(b);e.setReviver(function(f){}
);this.assertFunction(e.getReviver());}
,testReviverArray:function(){var g=new json.parse.decorator.components.StringData(b);g.setReviver([1,2,3]);this.assertArray(g.getReviver());}
}});}
)();
(function(){var a="",b='{"a":10,"b":[4,16],"c":true,"d":"string"}',c='parse_new.json',d='parse.json',e="string",f="json.test.integration.JsonTest",g='number',h="STRING";qx.Class.define(f,{extend:qx.dev.unit.TestCase,properties:{__tx:{},__cP:{},__tz:{},__tA:{},__tt:{}},members:{setUp:function(){this.__tz=this.__tx=function(i,j){if(typeof j===g){j=2*j;}
;return j;}
;this.__cP=b;this.__tt=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:e});this.__tA=0;}
,testFileToModel:function(){var k=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var m=k.join([d]);var l=json.Json.parse(m.getPath(),this.__tx,true,true);this.assertEquals(10,l.getA());}
,testFileToParse:function(){var n=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var p=n.join([d]);var o=json.Json.parse(p.getPath(),this.__tx,true);this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(o));}
,testStringToParse:function(){var q=json.Json.parse(this.__cP,this.__tx);this.assertEquals(String(JSON.parse(this.__cP,this.__tx)),String(q));}
,testStringToModel:function(){var r=json.Json.parse(this.__cP,this.__tx,false,true);this.assertEquals(20,r.getA());}
,testExternalBuilder:function(){var s=json.Json.parse(this.__cP,null,false,false,new json.test.integration.facade.ExternalBuilder(this.__cP));this.assertEquals(h,s.d);}
,testModelToJs:function(){var t={a:10,b:[4,16],c:true,d:e};var u=json.Json.stringify(this.__tt,this.__tz,this.__tA,false,a,true);this.assertEquals(JSON.stringify(t),JSON.stringify(u));}
,testModelToStringify:function(){var v={a:10,b:[4,16],c:true,d:e};var w=json.Json.stringify(this.__tt,this.__tz,this.__tA);this.assertEquals(JSON.stringify(v),w);}
,testModelToFile:function(){var x=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var A=x.join([c]);var y=b;var z=json.Json.stringify(this.__tt,this.__tz,this.__tA,true,A.getPath());this.assertEquals(y,z);}
}});}
)();
(function(){var a="",b="json.Json",c="static";qx.Class.define(b,{type:c,statics:{parse:function(e,f,g,h,d){f=f||null;h=h||false;g=g||false;d=d||null;var i=new json.facade.Parse();i.setValue(e);i.setReviver(f);i.setModel(h);i.setFile(g);if(d!==null){i.setBuilder(d);}
;return i.parse();}
,stringify:function(m,j,k,o,p,n,l){j=j||null;k=k||0;o=o||false;p=p||a;n=n||false;l=l||null;var q=new json.facade.Stringify();q.setValue(m);q.setReplacer(j);q.setSpace(k);q.setFile(o);q.setJsObject(n);q.setPath(p);if(l!==null){q.setBuilder(l);}
;return q.stringify();}
}});}
)();
(function(){var a='parse.json',b="json.test.integration.parse.builder.builders.BuilderTest",c='{"a":10,"b":[4,16],"c":true,"d":"string"}',d='number';qx.Class.define(b,{extend:qx.dev.unit.TestCase,properties:{__tx:{}},members:{__th:function(e){e.createComponent();e.buildReviver();e.buildDecorator();return e.getComponent();}
,setUp:function(){this.__tx=function(f,g){if(typeof g===d){g=2*g;}
;return g;}
;}
,testFileToParse:function(){var j=c;var i=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var l=i.join([a]);var h=new json.parse.builder.builders.FileToParse(l.getPath(),this.__tx);var k=this.__th(h);this.assertEquals(String(JSON.parse(j,this.__tx)),String(k.getData()));}
,testFileToModel:function(){var m=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var p=m.join([a]);var n=new json.parse.builder.builders.FileToModel(p.getPath(),this.__tx);var o=this.__th(n);this.assertEquals(10,o.getData().getA());}
,testStringToParse:function(){var r=c;var q=new json.parse.builder.builders.StringToParse(r,this.__tx);var s=this.__th(q);this.assertEquals(String(JSON.parse(r,this.__tx)),String(s.getData()));}
,testStringToModel:function(){var u=c;var t=new json.parse.builder.builders.StringToModel(u,this.__tx);var v=this.__th(t);this.assertEquals(20,v.getData().getA());}
}});}
)();
(function(){var a='parse_write.json',b="string",c="json.test.unit.stringify.decorator.decorators.WriteFileTest",d='{"a":5,"b":[2,8],"c":true,"d":"string"}';qx.Class.define(c,{extend:qx.dev.unit.TestCase,members:{testModel:function(){var k=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var f=k.join([a]);var l=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:b});var i=new json.stringify.decorator.components.ModelData(l);var j=new json.stringify.decorator.decorators.ModelToJs();j.setComponent(i);var e=new json.stringify.decorator.decorators.Stringify();e.setComponent(j);var g=new json.stringify.decorator.decorators.WriteFile(f.getPath());g.setComponent(e);var h=g.getData();this.assertEquals(d,h);}
}});}
)();
(function(){var a='parse_new.json',b="json.test.integration.stringify.builder.builders.BuilderTest",c="string",d='{"a":10,"b":[4,16],"c":true,"d":"string"}',e='number';qx.Class.define(b,{extend:qx.dev.unit.TestCase,properties:{__tz:{},__ts:{}},members:{__th:function(f){f.createComponent();f.buildReplacer();f.buildSpace();f.buildDecorator();return f.getComponent();}
,setUp:function(){this.__ts=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:c});this.__tz=function(g,h){if(typeof h===e){h=2*h;}
;return h;}
;}
,testModelToJs:function(){var j={a:10,b:[4,16],c:true,d:c};var i=new json.stringify.builder.builders.ModelToJs(this.__ts,this.__tz,0);var k=this.__th(i);this.assertEquals(JSON.stringify(j,this.__relacer),JSON.stringify(k.getData()));}
,testModelToStringify:function(){var m={a:10,b:[4,16],c:true,d:c};var l=new json.stringify.builder.builders.ModelToStringify(this.__ts,this.__tz,0);var n=this.__th(l);this.assertEquals(JSON.stringify(m,this.__relacer),n.getData());}
,testModelToFile:function(){var o=new criax.chromeless.lib.Path(criax.chromeless.lib.AppPath.getResourceDir());var s=o.join([a]);var q=d;var p=new json.stringify.builder.builders.ModelToFile(this.__ts,this.__tz,0);p.setPath(s.getPath());var r=this.__th(p);this.assertEquals(q,r.getData());}
}});}
)();
(function(){var a="json.test.unit.stringify.decorator.decorators.StringifyTest",b='{"a":10,"b":[4,16],"c":true,"d":"string"}',c='{\n  "a": 0\n}',d='number',e='{"a":5,"b":[2,8],"c":true,"d":"string"}',f="string";qx.Class.define(a,{extend:qx.dev.unit.TestCase,properties:{__ts:{},__tt:{},__tu:{}},members:{__th:function(){var g=new json.stringify.decorator.decorators.Stringify();g.setComponent(this.__tu);return g.getData();}
,setUp:function(){this.__ts=qx.data.marshal.Json.createModel({a:5,b:[2,8],c:true,d:f});this.__tt=new json.stringify.decorator.components.ModelData(this.__ts);this.__tu=new json.stringify.decorator.decorators.ModelToJs();}
,testModel:function(){this.__tu.setComponent(this.__tt);var h=this.__th();this.assertEquals(e,h);}
,testReplacer:function(){var i=function k(l,m){if(typeof m===d){m=2*m;}
;return m;}
;this.__tu.setComponent(this.__tt);this.__tu.setReplacer(i);var j=this.__th();this.assertEquals(b,j);}
,testSpace:function(){this.__ts=qx.data.marshal.Json.createModel({a:0});this.__tt=new json.stringify.decorator.components.ModelData(this.__ts);this.__tu=new json.stringify.decorator.decorators.ModelToJs();this.__tu.setComponent(this.__tt);this.__tu.setSpace(2);var n=this.__th();this.assertEquals(c,n);}
}});}
)();
