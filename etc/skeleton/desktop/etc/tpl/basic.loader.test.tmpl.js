(function() {
    ...
    qx.$$$$loader = {
        ...
        loadScriptList: function(uris) {
            var i, p, s;
            //CODIGO PARA TRANSFORMAR LAS RUTAS
            var appSourcePath = "${USER_APP}/${Name}/";
            var plataformSourcePath = "${ABS_QOOXDOO_PATH}/";
            var criaxSourcePath = "${ABS_QOOXDOO_PATH}/../../";
            for(var i in uris){                
                if(uris[i].slice(0,2) === './'){
                    uris[i] = uris[i].replace(uris[i].slice(0,2),appSourcePath+'test/script/');
                }else if(uris[i].slice(0,13) === '../../source/'){
                    uris[i] = uris[i].replace(uris[i].slice(0,13),appSourcePath+'source/');
                }else{
                    var partUri = uris[i].replace(/\.\.\//g,'');                    
                    if(partUri.indexOf('lib') !== -1){                        
                        var fullUri = criaxSourcePath + partUri;
                    }else{
                        var fullUri = plataformSourcePath + partUri;
                    }
                    var parts = fullUri.split("/");                    
                    var finalParts = [];
                    for(var j=0;j<parts.length;j++){                        
                        if(parts[j] === "testrunner" || parts[j] === ".."){
                            finalParts.push(parts[j]);
                        }else{
                            if(finalParts.indexOf(parts[j]) === -1){
                                finalParts.push(parts[j]);
                            }
                        }
                    }
                    uris[i] = finalParts.join("/");
                }                
            }
