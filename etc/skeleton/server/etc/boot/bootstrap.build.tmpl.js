...
var Bootstrap = function () { // open IIFE
    ...
    function initPlataformPath(){
        if(isLinuxOS){
            __plataformPath = environment['OLDPWD'].slice(0,-8);
        }else{
            __plataformPath = environment['plataformpath'].slice(1,-15).replace(/\//g,"\\");    
        }
        __plataformPath = __plataformPath.trim();
    }
    ...
    function initAppPath(){
        if(isLinuxOS){
            ...
            __appPath = environment["apppath"].slice(1, -8).replace(/\//g,"\\");    
        }
    ...
    function __serverPath(loadConfig){
        if(Boolean(loadConfig.path)){
            ...
            return __appPath + ((isLinuxOS) ? "/script" : "\\source\\script");
        }
    }
    ...
    function isValidRootDirectory(basePath){
        ...
        function server(host, port, basePath){
            ...
            let httpdFile = __plataformPath + ((isLinuxOS) ? "/webserver/httpd.js" : "\\webserver\\httpd.js");                
            load(httpdFile);
            DEBUG = true;
    ...
    Bootstrap.prototype.loadConfig = function(){
        let configFile = __appPath + ((isLinuxOS) ? "/../etc/configs/bootload.json" : "\\etc\\configs\\bootload.json");            
