...
:OKTYPE
if "%option%"=="generate" (
    start ${ABS_QOOXDOO_PATH}\..\..\bin\httpd\generate.bat %apppath% %plataformpath% ${Name}
    GOTO :INIT
)
if "%option%"=="update" (
    start ${ABS_QOOXDOO_PATH}\..\..\bin\httpd\update.bat %apppath% %plataformpath% ${Name}
    GOTO :INIT
)
if "%option%"=="run" (
    start ${ABS_QOOXDOO_PATH}\..\..\bin\httpd\run.bat %apppath% %plataformpath%
    GOTO :INIT
)
if "%option%"=="prebuild" (
    start ${ABS_QOOXDOO_PATH}\..\..\bin\httpd\prebuild.bat %apppath% %plataformpath% ${Name}
    GOTO :INIT
)
if "%option%"=="api" (
...
if "%option%"=="package" (
    start ${ABS_QOOXDOO_PATH}\..\..\bin\httpd\package.bat %apppath% %plataformpath% ${Name}
    GOTO :INIT
)
if "%option%"=="clean" (
