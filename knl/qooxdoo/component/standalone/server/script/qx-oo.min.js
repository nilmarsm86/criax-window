...
(function(){var b='!==inherit){',c='qx.lang.Type.isString(value) && qx.util.ColorUtil.isValidPropertyValue(value)',d='value !== null && qx.theme.manager.Font.getInstance().isDynamic(value)',e="set",f=';',g="resetThemed",h='value !== null && value.nodeType === 9 && value.documentElement',j='===value)return value;',k='value !== null && value.$$type === "Mixin"',m='return init;',n='var init=this.',o='value !== null && value.nodeType === 1 && value.attributes',p="var parent = this.getLayoutParent();",q="Error in property ",r='var a=this._getChildren();if(a)for(var i=0,l=a.length;i<l;i++){',s="();",t='.validate.call(this, value);',u='qx.core.Assert.assertInstance(value, Date, msg) || true',v='else{',w="if (!parent) return;",x=" in method ",y='qx.core.Assert.assertInstance(value, Error, msg) || true',z='=computed;',A='Undefined value is not allowed!',B='(backup);',C='else ',D='=true;',E='if(old===undefined)old=this.',F='if(computed===inherit){',G='old=computed=this.',H="inherit",I='if(this.',J='return this.',K='else if(this.',L='Is invalid!',M='if(value===undefined)prop.error(this,2,"',N='", "',O='var computed, old=this.',P='else if(computed===undefined)',Q='delete this.',R="resetRuntime",S="': ",T=" of class ",U='value !== null && value.nodeType !== undefined',V='===undefined)return;',W='value !== null && qx.theme.manager.Decoration.getInstance().isValidPropertyValue(value)',X="reset",Y="string",ba="')){",bb="module.events",bc="return this.",bd='qx.core.Assert.assertPositiveInteger(value, msg) || true',be='else this.',bf='value=this.',bg='","',bh='if(init==qx.core.Property.$$inherit)init=null;',bi="get",bj='value !== null && value.$$type === "Interface"',bk='var inherit=prop.$$inherit;',bl="', qx.event.type.Data, [computed, old]",bm="var value = parent.",bn="$$useinit_",bo='computed=undefined;delete this.',bp="(value);",bq='this.',br='Requires exactly one argument!',bs='",value);',bt='computed=value;',bu="$$runtime_",bv="setThemed",bw=';}',bx='(value);',by="$$user_",bz='!==undefined)',bA='){',bB='qx.core.Assert.assertArray(value, msg) || true',bC='if(computed===undefined||computed===inherit){',bD=";",bE='qx.core.Assert.assertPositiveNumber(value, msg) || true',bF=".prototype",bG="Boolean",bH=")}",bI="(a[",bJ='(computed, old, "',bK="setRuntime",bL='return value;',bM="this.",bN="if(reg.hasListener(this, '",bO='Does not allow any arguments!',bP=')a[i].',bQ="()",bR="var a=arguments[0] instanceof Array?arguments[0]:arguments;",bS='.$$properties.',bT='value !== null && value.$$type === "Theme"',bU='old=this.',bV="var reg=qx.event.Registration;",bW="())",bX='=value;',bY='return null;',ca='qx.core.Assert.assertObject(value, msg) || true',cb='");',cc='if(old===computed)return value;',cd='qx.core.Assert.assertString(value, msg) || true',ce='if(old===undefined)old=null;',cf='var pa=this.getLayoutParent();if(pa)computed=pa.',cg="if (value===undefined) value = parent.",ch='value !== null && value.$$type === "Class"',ci='qx.core.Assert.assertFunction(value, msg) || true',cj='!==undefined&&',ck='var computed, old;',cl='var backup=computed;',cm='}else{',cn='}',co="object",cp="$$init_",cq="$$theme_",cr='!==undefined){',cs='if(computed===undefined)computed=null;',ct="Unknown reason: ",cu="init",cv='qx.core.Assert.assertMap(value, msg) || true',cw='qx.core.Assert.assertNumber(value, msg) || true',cx='if((computed===undefined||computed===inherit)&&',cy="reg.fireEvent(this, '",cz='Null value is not allowed!',cA='qx.core.Assert.assertInteger(value, msg) || true',cB="value",cC="shorthand",cD='computed=this.',cE='qx.core.Assert.assertInstance(value, RegExp, msg) || true',cF='value !== null && value.type !== undefined',cG='value !== null && value.document',cH="",cI='throw new Error("Property ',cJ="(!this.",cK='qx.core.Assert.assertBoolean(value, msg) || true',cL='if(a[i].',cM=' of an instance of ',cN="toggle",cO="refresh",cP="$$inherit_",cQ='var prop=qx.core.Property;',cR="boolean",cS=" with incoming value '",cT="a=qx.lang.Array.fromShortHand(qx.lang.Array.fromArguments(a));",cU='if(computed===undefined||computed==inherit)computed=null;',cV="qx.core.Property",cW="is",cX=' is not (yet) ready!");',cY="]);",da='Could not change or apply init value after constructing phase!';qx.Bootstrap.define(cV,{statics:{__o:function(){if(qx.core.Environment.get(bb)){qx.event.type.Data;qx.event.dispatch.Direct;}
;}
,__p:{"Boolean":cK,"String":cd,"Number":cw,"Integer":cA,"PositiveNumber":bE,"PositiveInteger":bd,"Error":y,"RegExp":cE,"Object":ca,"Array":bB,"Map":cv,"Function":ci,"Date":u,"Node":U,"Element":o,"Document":h,"Window":cG,"Event":cF,"Class":ch,"Mixin":k,"Interface":bj,"Theme":bT,"Color":c,"Decorator":W,"Font":d},__q:{"Node":true,"Element":true,"Document":true,"Window":true,"Event":true},$$inherit:H,$$store:{runtime:{},user:{},theme:{},inherit:{},init:{},useinit:{}},$$method:{get:{},set:{},reset:{},init:{},refresh:{},setRuntime:{},resetRuntime:{},setThemed:{},resetThemed:{}},$$allowedKeys:{name:Y,dereference:cR,inheritable:cR,nullable:cR,themeable:cR,refine:cR,init:null,apply:Y,event:Y,check:null,transform:Y,deferredInit:cR,validate:null},$$allowedGroupKeys:{name:Y,group:co,mode:Y,themeable:cR},$$inheritable:{},__r:function(dd){var db=this.__s(dd);if(!db.length){var dc=function(){}
;}
else {dc=this.__t(db);}
;dd.prototype.$$refreshInheritables=dc;}
,__s:function(de){var df=[];while(de){var dg=de.$$properties;if(dg){for(var name in this.$$inheritable){if(dg[name]&&dg[name].inheritable){df.push(name);}
;}
;}
;de=de.superclass;}
;return df;}
,__t:function(inheritables){var inherit=this.$$store.inherit;var init=this.$$store.init;var refresh=this.$$method.refresh;var code=[p,w];for(var i=0,l=inheritables.length;i<l;i++ ){var name=inheritables[i];code.push(bm,inherit[name],bD,cg,init[name],bD,bM,refresh[name],bp);}
;return new Function(code.join(cH));}
,attachRefreshInheritables:function(dh){dh.prototype.$$refreshInheritables=function(){qx.core.Property.__r(dh);return this.$$refreshInheritables();}
;}
,attachMethods:function(dj,name,di){di.group?this.__u(dj,di,name):this.__v(dj,di,name);}
...
;var argHandler=bR;setter.push(argHandler);if(themeable){styler.push(argHandler);}
;if(config.mode==cC){var shorthand=cT;setter.push(shorthand);if(themeable){styler.push(shorthand);}
;}
;for(var i=0,a=config.group,l=a.length;i<l;i++ ){{}
;setter.push(bM,this.$$method.set[a[i]],bI,i,cY);resetter.push(bM,this.$$method.reset[a[i]],s);if(themeable){{}
;styler.push(bM,this.$$method.setThemed[a[i]],bI,i,cY);unstyler.push(bM,this.$$method.resetThemed[a[i]],s);}
;}
;this.$$method.set[name]=e+upname;members[this.$$method.set[name]]=new Function(setter.join(cH));this.$$method.reset[name]=X+upname;members[this.$$method.reset[name]]=new Function(resetter.join(cH));if(themeable){this.$$method.setThemed[name]=bv+upname;members[this.$$method.setThemed[name]]=new Function(styler.join(cH));this.$$method.resetThemed[name]=g+upname;members[this.$$method.resetThemed[name]]=new Function(unstyler.join(cH));}
;}
...
;var method=this.$$method;var store=this.$$store;store.runtime[name]=bu+name;store.user[name]=by+name;store.theme[name]=cq+name;store.init[name]=cp+name;store.inherit[name]=cP+name;store.useinit[name]=bn+name;method.get[name]=bi+upname;members[method.get[name]]=function(){return qx.core.Property.executeOptimizedGetter(this,clazz,name,bi);}
;method.set[name]=e+upname;members[method.set[name]]=function(dk){return qx.core.Property.executeOptimizedSetter(this,clazz,name,e,arguments);}
...
;if(config.inheritable||config.apply||config.event||config.deferredInit){method.init[name]=cu+upname;members[method.init[name]]=function(dl){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cu,arguments);}
;{}
;}
;if(config.inheritable){method.refresh[name]=cO+upname;members[method.refresh[name]]=function(dm){return qx.core.Property.executeOptimizedSetter(this,clazz,name,cO,arguments);}
;{}
;}
;method.setRuntime[name]=bK+upname;members[method.setRuntime[name]]=function(dn){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bK,arguments);}
...
;if(config.themeable){method.setThemed[name]=bv+upname;members[method.setThemed[name]]=function(dp){return qx.core.Property.executeOptimizedSetter(this,clazz,name,bv,arguments);}
...
;{}
;}
;if(config.check===bG){members[cN+upname]=new Function(bc+method.set[name]+cJ+method.get[name]+bW);members[cW+upname]=new Function(bc+method.get[name]+bQ);{}
;}
;{}
;}
,__w:function(dq){return !!this.__q[dq];}
,__x:{'0':da,'1':br,'2':A,'3':bO,'4':cz,'5':L},error:function(dr,dx,dw,ds,dt){var du=dr.constructor.classname;var dv=q+dw+T+du+x+this.$$method[ds][dw]+cS+dt+S;throw new Error(dv+(this.__x[dx]||ct+dx));}
,__y:function(instance,members,name,variant,code,args){var store=this.$$method[variant][name];{members[store]=new Function(cB,code.join(cH));}
;{}
...
;}
,executeOptimizedGetter:function(dB,dA,name,dz){var dD=dA.$$properties[name];var dC=dA.prototype;var dy=[];var dE=this.$$store;dy.push(I,dE.runtime[name],bz);dy.push(J,dE.runtime[name],f);if(dD.inheritable){dy.push(K,dE.inherit[name],bz);dy.push(J,dE.inherit[name],f);dy.push(C);}
;dy.push(I,dE.user[name],bz);dy.push(J,dE.user[name],f);if(dD.themeable){dy.push(K,dE.theme[name],bz);dy.push(J,dE.theme[name],f);}
;if(dD.deferredInit&&dD.init===undefined){dy.push(K,dE.init[name],bz);dy.push(J,dE.init[name],f);}
;dy.push(C);if(dD.init!==undefined){if(dD.inheritable){dy.push(n,dE.init[name],f);if(dD.nullable){dy.push(bh);}
;dy.push(m);}
else {dy.push(J,dE.init[name],f);}
;}
else if(dD.inheritable||dD.nullable){dy.push(bY);}
else {dy.push(cI,name,cM,dA.classname,cX);}
;return this.__y(dB,dC,name,dz,dy);}
,executeOptimizedSetter:function(dL,dK,name,dJ,dI){var dN=dK.$$properties[name];var dM=dK.prototype;var dG=[];var dF=dJ===e||dJ===bv||dJ===bK||(dJ===cu&&dN.init===undefined);var dH=dN.apply||dN.event||dN.inheritable;var dO=this.__z(dJ,name);this.__A(dG,dN,name,dJ,dF);if(dF){this.__B(dG,dK,dN,name);}
;if(dH){this.__C(dG,dF,dO,dJ);}
;if(dN.inheritable){dG.push(bk);}
;{}
;if(!dH){this.__E(dG,name,dJ,dF);}
else {this.__F(dG,dN,name,dJ,dF);}
;if(dN.inheritable){this.__G(dG,dN,name,dJ);}
else if(dH){this.__H(dG,dN,name,dJ);}
;if(dH){this.__I(dG,dN,name,dJ);if(dN.inheritable&&dM._getChildren){this.__J(dG,name);}
;}
;if(dF){dG.push(bL);}
;return this.__y(dL,dM,name,dJ,dG,dI);}
,__z:function(dP,name){if(dP===bK||dP===R){var dQ=this.$$store.runtime[name];}
else if(dP===bv||dP===g){dQ=this.$$store.theme[name];}
else if(dP===cu){dQ=this.$$store.init[name];}
else {dQ=this.$$store.user[name];}
;return dQ;}
,__A:function(dT,dR,name,dU,dS){{if(!dR.nullable||dR.check||dR.inheritable){dT.push(cQ);}
;if(dU===e){dT.push(M,name,bg,dU,bs);}
;}
;}
,__B:function(dV,dX,dW,name){if(dW.transform){dV.push(bf,dW.transform,bx);}
;if(dW.validate){if(typeof dW.validate===Y){dV.push(bq,dW.validate,bx);}
else if(dW.validate instanceof Function){dV.push(dX.classname,bS,name);dV.push(t);}
;}
;}
,__C:function(ea,dY,ec,eb){var ed=(eb===X||eb===g||eb===R);if(dY){ea.push(I,ec,j);}
else if(ed){ea.push(I,ec,V);}
;}
,__D:undefined,__E:function(ef,name,eg,ee){if(eg===bK){ef.push(bq,this.$$store.runtime[name],bX);}
else if(eg===R){ef.push(I,this.$$store.runtime[name],bz);ef.push(Q,this.$$store.runtime[name],f);}
else if(eg===e){ef.push(bq,this.$$store.user[name],bX);}
else if(eg===X){ef.push(I,this.$$store.user[name],bz);ef.push(Q,this.$$store.user[name],f);}
else if(eg===bv){ef.push(bq,this.$$store.theme[name],bX);}
else if(eg===g){ef.push(I,this.$$store.theme[name],bz);ef.push(Q,this.$$store.theme[name],f);}
else if(eg===cu&&ee){ef.push(bq,this.$$store.init[name],bX);}
;}
,__F:function(ej,eh,name,ek,ei){if(eh.inheritable){ej.push(O,this.$$store.inherit[name],f);}
else {ej.push(ck);}
;ej.push(I,this.$$store.runtime[name],cr);if(ek===bK){ej.push(cD,this.$$store.runtime[name],bX);}
else if(ek===R){ej.push(Q,this.$$store.runtime[name],f);ej.push(I,this.$$store.user[name],bz);ej.push(cD,this.$$store.user[name],f);ej.push(K,this.$$store.theme[name],bz);ej.push(cD,this.$$store.theme[name],f);ej.push(K,this.$$store.init[name],cr);ej.push(cD,this.$$store.init[name],f);ej.push(bq,this.$$store.useinit[name],D);ej.push(cn);}
else {ej.push(G,this.$$store.runtime[name],f);if(ek===e){ej.push(bq,this.$$store.user[name],bX);}
else if(ek===X){ej.push(Q,this.$$store.user[name],f);}
else if(ek===bv){ej.push(bq,this.$$store.theme[name],bX);}
else if(ek===g){ej.push(Q,this.$$store.theme[name],f);}
else if(ek===cu&&ei){ej.push(bq,this.$$store.init[name],bX);}
;}
;ej.push(cn);ej.push(K,this.$$store.user[name],cr);if(ek===e){if(!eh.inheritable){ej.push(bU,this.$$store.user[name],f);}
;ej.push(cD,this.$$store.user[name],bX);}
else if(ek===X){if(!eh.inheritable){ej.push(bU,this.$$store.user[name],f);}
;ej.push(Q,this.$$store.user[name],f);ej.push(I,this.$$store.runtime[name],bz);ej.push(cD,this.$$store.runtime[name],f);ej.push(I,this.$$store.theme[name],bz);ej.push(cD,this.$$store.theme[name],f);ej.push(K,this.$$store.init[name],cr);ej.push(cD,this.$$store.init[name],f);ej.push(bq,this.$$store.useinit[name],D);ej.push(cn);}
else {if(ek===bK){ej.push(cD,this.$$store.runtime[name],bX);}
else if(eh.inheritable){ej.push(cD,this.$$store.user[name],f);}
else {ej.push(G,this.$$store.user[name],f);}
;if(ek===bv){ej.push(bq,this.$$store.theme[name],bX);}
else if(ek===g){ej.push(Q,this.$$store.theme[name],f);}
else if(ek===cu&&ei){ej.push(bq,this.$$store.init[name],bX);}
;}
;ej.push(cn);if(eh.themeable){ej.push(K,this.$$store.theme[name],cr);if(!eh.inheritable){ej.push(bU,this.$$store.theme[name],f);}
;if(ek===bK){ej.push(cD,this.$$store.runtime[name],bX);}
else if(ek===e){ej.push(cD,this.$$store.user[name],bX);}
else if(ek===bv){ej.push(cD,this.$$store.theme[name],bX);}
else if(ek===g){ej.push(Q,this.$$store.theme[name],f);ej.push(I,this.$$store.init[name],cr);ej.push(cD,this.$$store.init[name],f);ej.push(bq,this.$$store.useinit[name],D);ej.push(cn);}
else if(ek===cu){if(ei){ej.push(bq,this.$$store.init[name],bX);}
;ej.push(cD,this.$$store.theme[name],f);}
else if(ek===cO){ej.push(cD,this.$$store.theme[name],f);}
;ej.push(cn);}
;ej.push(K,this.$$store.useinit[name],bA);if(!eh.inheritable){ej.push(bU,this.$$store.init[name],f);}
;if(ek===cu){if(ei){ej.push(cD,this.$$store.init[name],bX);}
else {ej.push(cD,this.$$store.init[name],f);}
;}
else if(ek===e||ek===bK||ek===bv||ek===cO){ej.push(Q,this.$$store.useinit[name],f);if(ek===bK){ej.push(cD,this.$$store.runtime[name],bX);}
else if(ek===e){ej.push(cD,this.$$store.user[name],bX);}
else if(ek===bv){ej.push(cD,this.$$store.theme[name],bX);}
else if(ek===cO){ej.push(cD,this.$$store.init[name],f);}
;}
;ej.push(cn);if(ek===e||ek===bK||ek===bv||ek===cu){ej.push(v);if(ek===bK){ej.push(cD,this.$$store.runtime[name],bX);}
else if(ek===e){ej.push(cD,this.$$store.user[name],bX);}
else if(ek===bv){ej.push(cD,this.$$store.theme[name],bX);}
else if(ek===cu){if(ei){ej.push(cD,this.$$store.init[name],bX);}
else {ej.push(cD,this.$$store.init[name],f);}
;ej.push(bq,this.$$store.useinit[name],D);}
;ej.push(cn);}
;}
,__G:function(em,el,name,en){em.push(bC);if(en===cO){em.push(bt);}
else {em.push(cf,this.$$store.inherit[name],f);}
;em.push(cx);em.push(bq,this.$$store.init[name],cj);em.push(bq,this.$$store.init[name],b);em.push(cD,this.$$store.init[name],f);em.push(bq,this.$$store.useinit[name],D);em.push(cm);em.push(Q,this.$$store.useinit[name],bw);em.push(cn);em.push(cc);em.push(F);em.push(bo,this.$$store.inherit[name],f);em.push(cn);em.push(P);em.push(Q,this.$$store.inherit[name],f);em.push(be,this.$$store.inherit[name],z);em.push(cl);if(el.init!==undefined&&en!==cu){em.push(E,this.$$store.init[name],bD);}
else {em.push(ce);}
;em.push(cU);}
,__H:function(ep,eo,name,eq){if(eq!==e&&eq!==bK&&eq!==bv){ep.push(cs);}
;ep.push(cc);if(eo.init!==undefined&&eq!==cu){ep.push(E,this.$$store.init[name],bD);}
else {ep.push(ce);}
;}
,__I:function(es,er,name,et){if(er.apply){es.push(bq,er.apply,bJ,name,N,et,cb);}
;if(er.event){es.push(bV,bN,er.event,ba,cy,er.event,bl,bH);}
;}
,__J:function(eu,name){eu.push(r);eu.push(cL,this.$$method.refresh[name],bP,this.$$method.refresh[name],B);eu.push(cn);}
}});}
)();
...
(function(){var a=". Error message: ",b="Boolean",c=").",d="set",f="deepBinding",g=") to the object '",h="item",k="Please use only one array at a time: ",l="Integer",m="reset",n=" of object ",p="qx.data.SingleValueBinding",q="Binding property ",r="Failed so set value ",s="change",t="Binding could not be found!",u="get",v="^",w=" does not work.",x="String",y="Binding from '",z="",A="PositiveNumber",B="]",C="[",D=".",E="PositiveInteger",F='No number or \'last\' value hast been given in an array binding: ',G="' (",H=" on ",I="Binding does not exist!",J="Number",K=".[",L="Date",M=" not possible: No event available. ",N="last";qx.Class.define(p,{statics:{__bb:{},__bc:{},bind:function(bb,U,bg,bd,bj){{}
;var bf=this.__be(bb,U,bg,bd,bj);var Q=U.split(D);var P=this.__bm(Q);var S=[];var bh=[];var R=[];var T=[];var Y=bb;try{for(var i=0;i<Q.length;i++ ){if(P[i]!==z){T.push(s);}
else {var X=this.__bf(Y,Q[i]);if(!X){if(i==0){throw new qx.core.AssertionError(q+Q[i]+n+Y+M);}
;this.__bl(undefined,bg,bd,bj,bb);break;}
;T.push(X);}
;S[i]=Y;if(i==Q.length-1){if(P[i]!==z){var ba=P[i]===N?Y.length-1:P[i];var O=Y.getItem(ba);this.__bl(O,bg,bd,bj,bb);R[i]=this.__bn(Y,T[i],bg,bd,bj,P[i]);}
else {if(Q[i]!=null&&Y[u+qx.lang.String.firstUp(Q[i])]!=null){var O=Y[u+qx.lang.String.firstUp(Q[i])]();this.__bl(O,bg,bd,bj,bb);}
;R[i]=this.__bn(Y,T[i],bg,bd,bj);}
;}
else {var bi={index:i,propertyNames:Q,sources:S,listenerIds:R,arrayIndexValues:P,targetObject:bg,targetPropertyChain:bd,options:bj,listeners:bh};var be=qx.lang.Function.bind(this.__bd,this,bi);bh.push(be);R[i]=Y.addListener(T[i],be);}
;if(Y[u+qx.lang.String.firstUp(Q[i])]==null){Y=undefined;}
else if(P[i]!==z){var ba=P[i]===N?Y.length-1:P[i];Y=Y[u+qx.lang.String.firstUp(Q[i])](ba);}
else {Y=Y[u+qx.lang.String.firstUp(Q[i])]();if(Y===null&&(Q.length-1)!=i){Y=undefined;}
;}
;if(!Y){this.__bl(Y,bg,bd,bj,bb);break;}
;}
;}
catch(bk){for(var i=0;i<S.length;i++ ){if(S[i]&&R[i]){S[i].removeListenerById(R[i]);}
;}
;var V=bf.targets;var bc=bf.listenerIds;for(var i=0;i<V.length;i++ ){if(V[i]&&bc[i]){V[i].removeListenerById(bc[i]);}
;}
;throw bk;}
;var W={type:f,listenerIds:R,sources:S,targetListenerIds:bf.listenerIds,targets:bf.targets};this.__bo(W,bb,U,bg,bd);return W;}
,__bd:function(br){if(br.options&&br.options.onUpdate){br.options.onUpdate(br.sources[br.index],br.targetObject);}
;for(var j=br.index+1;j<br.propertyNames.length;j++ ){var bp=br.sources[j];br.sources[j]=null;if(!bp){continue;}
;bp.removeListenerById(br.listenerIds[j]);}
;var bp=br.sources[br.index];for(var j=br.index+1;j<br.propertyNames.length;j++ ){if(br.arrayIndexValues[j-1]!==z){bp=bp[u+qx.lang.String.firstUp(br.propertyNames[j-1])](br.arrayIndexValues[j-1]);}
else {bp=bp[u+qx.lang.String.firstUp(br.propertyNames[j-1])]();}
;br.sources[j]=bp;if(!bp){if(br.options&&br.options.converter){var bl=false;if(br.options.ignoreConverter){var bs=br.propertyNames.slice(0,j).join(D);var bq=bs.match(new RegExp(v+br.options.ignoreConverter));bl=bq?bq.length>0:false;}
;if(!bl){this.__bh(br.targetObject,br.targetPropertyChain,br.options.converter());}
else {this.__bg(br.targetObject,br.targetPropertyChain);}
;}
else {this.__bg(br.targetObject,br.targetPropertyChain);}
;break;}
;if(j==br.propertyNames.length-1){if(qx.Class.implementsInterface(bp,qx.data.IListData)){var bt=br.arrayIndexValues[j]===N?bp.length-1:br.arrayIndexValues[j];var bm=bp.getItem(bt);this.__bl(bm,br.targetObject,br.targetPropertyChain,br.options,br.sources[br.index]);br.listenerIds[j]=this.__bn(bp,s,br.targetObject,br.targetPropertyChain,br.options,br.arrayIndexValues[j]);}
else {if(br.propertyNames[j]!=null&&bp[u+qx.lang.String.firstUp(br.propertyNames[j])]!=null){var bm=bp[u+qx.lang.String.firstUp(br.propertyNames[j])]();this.__bl(bm,br.targetObject,br.targetPropertyChain,br.options,br.sources[br.index]);}
;var bn=this.__bf(bp,br.propertyNames[j]);if(!bn){this.__bg(br.targetObject,br.targetPropertyChain);break;}
;br.listenerIds[j]=this.__bn(bp,bn,br.targetObject,br.targetPropertyChain,br.options);}
;}
else {if(br.listeners[j]==null){var bo=qx.lang.Function.bind(this.__bd,this,br);br.listeners.push(bo);}
;if(qx.Class.implementsInterface(bp,qx.data.IListData)){var bn=s;}
else {var bn=this.__bf(bp,br.propertyNames[j]);}
;if(!bn){this.__bg(br.targetObject,br.targetPropertyChain);return;}
;br.listenerIds[j]=bp.addListener(bn,br.listeners[j]);}
;}
;}
,__be:function(bw,bE,bI,bA,bC){var bz=bA.split(D);var bx=this.__bm(bz);var bH=[];var bG=[];var bB=[];var bF=[];var by=bI;for(var i=0;i<bz.length-1;i++ ){if(bx[i]!==z){bF.push(s);}
else {var bv=this.__bf(by,bz[i]);if(!bv){break;}
;bF.push(bv);}
;bH[i]=by;var bD=function(){for(var j=i+1;j<bz.length-1;j++ ){var bL=bH[j];bH[j]=null;if(!bL){continue;}
;bL.removeListenerById(bB[j]);}
;var bL=bH[i];for(var j=i+1;j<bz.length-1;j++ ){var bJ=qx.lang.String.firstUp(bz[j-1]);if(bx[j-1]!==z){var bM=bx[j-1]===N?bL.getLength()-1:bx[j-1];bL=bL[u+bJ](bM);}
else {bL=bL[u+bJ]();}
;bH[j]=bL;if(bG[j]==null){bG.push(bD);}
;if(qx.Class.implementsInterface(bL,qx.data.IListData)){var bK=s;}
else {var bK=qx.data.SingleValueBinding.__bf(bL,bz[j]);if(!bK){break;}
;}
;bB[j]=bL.addListener(bK,bG[j]);}
;qx.data.SingleValueBinding.updateTarget(bw,bE,bI,bA,bC);}
;bG.push(bD);bB[i]=by.addListener(bF[i],bD);var bu=qx.lang.String.firstUp(bz[i]);if(by[u+bu]==null){by=null;}
else if(bx[i]!==z){by=by[u+bu](bx[i]);}
else {by=by[u+bu]();}
;if(!by){break;}
;}
;return {listenerIds:bB,targets:bH};}
,updateTarget:function(bN,bQ,bS,bO,bR){var bP=this.resolvePropertyChain(bN,bQ);bP=qx.data.SingleValueBinding.__bp(bP,bS,bO,bR,bN);this.__bh(bS,bO,bP);}
,resolvePropertyChain:function(o,bT){var bU=this.__bj(bT);return this.__bk(o,bU,bU.length);}
,__bf:function(bW,bX){var bV=this.__bq(bW,bX);if(bV==null){if(qx.Class.supportsEvent(bW.constructor,bX)){bV=bX;}
else if(qx.Class.supportsEvent(bW.constructor,s+qx.lang.String.firstUp(bX))){bV=s+qx.lang.String.firstUp(bX);}
else {return null;}
;}
;return bV;}
,__bg:function(cd,cb){var cc=this.__bj(cb);var ca=this.__bk(cd,cc);if(ca!=null){var ce=cc[cc.length-1];var bY=this.__bi(ce);if(bY){this.__bh(cd,cb,null);return;}
;if(ca[m+qx.lang.String.firstUp(ce)]!=undefined){ca[m+qx.lang.String.firstUp(ce)]();}
else {ca[d+qx.lang.String.firstUp(ce)](null);}
;}
;}
,__bh:function(ck,ch,ci){var cj=this.__bj(ch);var cg=this.__bk(ck,cj);if(cg){var cl=cj[cj.length-1];var cf=this.__bi(cl);if(cf){if(cf===N){cf=cg.length-1;}
;cg.setItem(cf,ci);}
else {cg[d+qx.lang.String.firstUp(cl)](ci);}
;}
;}
,__bi:function(co){var cm=/^\[(\d+|last)\]$/;var cn=co.match(cm);if(cn){return cn[1];}
;return null;}
,__bj:function(cp){return cp.replace(/\[/g,K).split(D).filter(function(cq){return cq!==z;}
);}
,__bk:function(cw,cr,cs){cs=cs||cr.length-1;var cu=cw;for(var i=0;i<cs;i++ ){try{var cv=cr[i];var ct=this.__bi(cv);if(ct){if(ct===N){ct=cu.length-1;}
;cu=cu.getItem(ct);}
else {cu=cu[u+qx.lang.String.firstUp(cv)]();}
;}
catch(cx){return null;}
;}
;return cu;}
,__bl:function(cC,cy,cA,cB,cz){cC=this.__bp(cC,cy,cA,cB,cz);if(cC===undefined){this.__bg(cy,cA);}
;if(cC!==undefined){try{this.__bh(cy,cA,cC);if(cB&&cB.onUpdate){cB.onUpdate(cz,cy,cC);}
;}
catch(e){if(!(e instanceof qx.core.ValidationError)){throw e;}
;if(cB&&cB.onSetFail){cB.onSetFail(e);}
else {qx.log.Logger.warn(r+cC+H+cy+a+e);}
;}
;}
;}
,__bm:function(cD){var cE=[];for(var i=0;i<cD.length;i++ ){var name=cD[i];if(qx.lang.String.endsWith(name,B)){var cF=name.substring(name.indexOf(C)+1,name.indexOf(B));if(name.indexOf(B)!=name.length-1){throw new Error(k+name+w);}
;if(cF!==N){if(cF==z||isNaN(parseInt(cF,10))){throw new Error(F+name+w);}
;}
;if(name.indexOf(C)!=0){cD[i]=name.substring(0,name.indexOf(C));cE[i]=z;cE[i+1]=cF;cD.splice(i+1,0,h);i++ ;}
else {cE[i]=cF;cD.splice(i,1,h);}
;}
else {cE[i]=z;}
;}
;return cE;}
,__bn:function(cG,cJ,cO,cM,cK,cI){{var cH;}
;var cL=function(cR,e){if(cR!==z){if(cR===N){cR=cG.length-1;}
;var cS=cG.getItem(cR);if(cS===undefined){qx.data.SingleValueBinding.__bg(cO,cM);}
;var cQ=e.getData().start;var cP=e.getData().end;if(cR<cQ||cR>cP){return;}
;}
else {var cS=e.getData();}
;{}
;cS=qx.data.SingleValueBinding.__bp(cS,cO,cM,cK,cG);{}
;try{if(cS!==undefined){qx.data.SingleValueBinding.__bh(cO,cM,cS);}
else {qx.data.SingleValueBinding.__bg(cO,cM);}
;if(cK&&cK.onUpdate){cK.onUpdate(cG,cO,cS);}
;}
catch(cT){if(!(cT instanceof qx.core.ValidationError)){throw cT;}
;if(cK&&cK.onSetFail){cK.onSetFail(cT);}
else {qx.log.Logger.warn(r+cS+H+cO+a+cT);}
;}
;}
;if(!cI){cI=z;}
;cL=qx.lang.Function.bind(cL,cG,cI);var cN=cG.addListener(cJ,cL);return cN;}
,__bo:function(da,cU,cX,db,cY){var cV;cV=cU.toHashCode();if(this.__bb[cV]===undefined){this.__bb[cV]=[];}
;var cW=[da,cU,cX,db,cY];this.__bb[cV].push(cW);cV=db.toHashCode();if(this.__bc[cV]===undefined){this.__bc[cV]=[];}
;this.__bc[cV].push(cW);}
,__bp:function(df,dl,de,dh,dc){if(dh&&dh.converter){var di;if(dl.getModel){di=dl.getModel();}
;return dh.converter(df,di,dc,dl);}
else {var dg=this.__bj(de);var dd=this.__bk(dl,dg);var dm=de.substring(de.lastIndexOf(D)+1,de.length);if(dd==null){return df;}
;var dj=qx.Class.getPropertyDefinition(dd.constructor,dm);var dk=dj==null?z:dj.check;return this.__br(df,dk);}
;}
,__bq:function(dn,dq){var dp=qx.Class.getPropertyDefinition(dn.constructor,dq);if(dp==null){return null;}
;return dp.event;}
,__br:function(dt,ds){var dr=qx.lang.Type.getClass(dt);if((dr==J||dr==x)&&(ds==l||ds==E)){dt=parseInt(dt,10);}
;if((dr==b||dr==J||dr==L)&&ds==x){dt=dt+z;}
;if((dr==J||dr==x)&&(ds==J||ds==A)){dt=parseFloat(dt);}
;return dt;}
,removeBindingFromObject:function(du,dy){if(dy.type==f){for(var i=0;i<dy.sources.length;i++ ){if(dy.sources[i]){dy.sources[i].removeListenerById(dy.listenerIds[i]);}
;}
...
;var dx=this.getAllBindingsForObject(du);if(dx!=undefined){for(var i=0;i<dx.length;i++ ){if(dx[i][0]==dy){var dv=dx[i][3];if(this.__bc[dv.toHashCode()]){qx.lang.Array.remove(this.__bc[dv.toHashCode()],dx[i]);}
;var dw=dx[i][1];if(this.__bb[dw.toHashCode()]){qx.lang.Array.remove(this.__bb[dw.toHashCode()],dx[i]);}
;return;}
;}
;}
;throw new Error(t);}
,removeAllBindingsForObject:function(dA){{}
;var dz=this.getAllBindingsForObject(dA);if(dz!=undefined){for(var i=dz.length-1;i>=0;i-- ){this.removeBindingFromObject(dA,dz[i][0]);}
;}
;}
,removeRelatedBindings:function(dC,dD){{}
;var dF=this.getAllBindingsForObject(dC);if(dF!=undefined){for(var i=dF.length-1;i>=0;i-- ){var dE=dF[i][1];var dB=dF[i][3];if(dE===dD||dB===dD){this.removeBindingFromObject(dC,dF[i][0]);}
;}
;}
;}
,getAllBindingsForObject:function(dH){var dI=dH.toHashCode();if(this.__bb[dI]===undefined){this.__bb[dI]=[];}
;var dJ=this.__bb[dI];var dG=this.__bc[dI]?this.__bc[dI]:[];return qx.lang.Array.unique(dJ.concat(dG));}
,removeAllBindings:function(){for(var dL in this.__bb){var dK=qx.core.ObjectRegistry.fromHashCode(dL);if(dK==null){delete this.__bb[dL];continue;}
;this.removeAllBindingsForObject(dK);}
;this.__bb={};}
,getAllBindings:function(){return this.__bb;}
,showBindingInLog:function(dN,dP){var dO;for(var i=0;i<this.__bb[dN.toHashCode()].length;i++ ){if(this.__bb[dN.toHashCode()][i][0]==dP){dO=this.__bb[dN.toHashCode()][i];break;}
;}
;if(dO===undefined){var dM=I;}
else {var dM=y+dO[1]+G+dO[2]+g+dO[3]+G+dO[4]+c;}
;qx.log.Logger.debug(dM);}
,showAllBindingsInLog:function(){for(var dR in this.__bb){var dQ=qx.core.ObjectRegistry.fromHashCode(dR);for(var i=0;i<this.__bb[dR].length;i++ ){this.showBindingInLog(dQ,this.__bb[dR][i][0]);}
;}
;}
}});}
)();
...
;R=/@(.+):(\d+)$/gm;while((S=R.exec(bd.stack))!=null){bb=S[1];Q=S[2];ba=this.__bv(bb);T.push(ba+m+Q);}
;if(T.length>0){return this.__bx(T);}
;R=/at (.*)/gm;var be=/\((.*?)(:[^\/].*)\)/;var Y=/(.*?)(:[^\/].*)/;while((S=R.exec(bd.stack))!=null){var X=be.exec(S[1]);if(!X){X=Y.exec(S[1]);}
;if(X){ba=this.__bv(X[1]);T.push(ba+X[2]);}
else {T.push(S[1]);}
;}
;}
else if(bc===q){var U=bd.stacktrace;if(!U){return T;}
;if(U.indexOf(h)>=0){U=U.split(h)[0];}
;R=/line\ (\d+?),\ column\ (\d+?)\ in\ (?:.*?)\ in\ (.*?):[^\/]/gm;while((S=R.exec(U))!=null){Q=S[1];P=S[2];bb=S[3];ba=this.__bv(bb);T.push(ba+m+Q+m+P);}
;if(T.length>0){return this.__bx(T);}
;R=/Line\ (\d+?)\ of\ linked\ script\ (.*?)$/gm;while((S=R.exec(U))!=null){Q=S[1];bb=S[2];ba=this.__bv(bb);T.push(ba+m+Q);}
;}
else if(bd.message&&bd.message.indexOf(k)>=0){var W=bd.message.split(k)[1].trim();var V=W.split(e);for(var i=0;i<V.length;i++ ){var O=V[i].match(/\s*Line ([0-9]+) of.* (\S.*)/);if(O&&O.length>=2){Q=O[1];bf=this.__bv(O[2]);T.push(bf+m+Q);}
;}
;}
else if(bd.sourceURL&&bd.line){T.push(this.__bv(bd.sourceURL)+m+bd.line);}
;return this.__bx(T);}
,__bv:function(bh){if(typeof qx.dev.StackTrace.FILENAME_TO_CLASSNAME==o){var bg=qx.dev.StackTrace.FILENAME_TO_CLASSNAME(bh);{}
;return bg;}
;return qx.dev.StackTrace.__bw(bh);}
,__bw:function(bk){var bl=g;var bi=bk.indexOf(bl);var bm=bk.indexOf(f);if(bm>=0){bk=bk.substring(0,bm);}
;var bj=(bi==-1)?bk:bk.substring(bi+bl.length).replace(/\//g,n).replace(/\.js$/,d);return bj;}
,__bx:function(bn){if(typeof qx.dev.StackTrace.FORMAT_STACKTRACE==o){bn=qx.dev.StackTrace.FORMAT_STACKTRACE(bn);{}
;}
;return bn;}
...
,__bB:function(){return (window.navigator.mozApps||window.navigator.buildID)&&window.navigator.product===k&&window.navigator.userAgent.indexOf(l)==-1&&window.navigator.userAgent.indexOf(n)==-1;}
...
(function(){var c="-",d="",e="qx.core.ObjectRegistry",f="Disposed ",g="$$hash",h="-0",j=" objects",k="Could not dispose object ",m=": ";qx.Bootstrap.define(e,{statics:{inShutDown:false,__bW:{},__bX:0,__bY:[],__ca:d,__cb:{},register:function(n){var q=this.__bW;if(!q){return;}
;var p=n.$$hash;if(p==null){var o=this.__bY;if(o.length>0&&true){p=o.pop();}
else {p=(this.__bX++ )+this.__ca;}
;n.$$hash=p;{}
;}
;{}
;q[p]=n;}
,unregister:function(r){var s=r.$$hash;if(s==null){return;}
;var t=this.__bW;if(t&&t[s]){delete t[s];this.__bY.push(s);}
;try{delete r.$$hash;}
...
;}
;}
,fromHashCode:function(B){return this.__bW[B]||null;}
,shutdown:function(){this.inShutDown=true;var D=this.__bW;var F=[];for(var C in D){F.push(C);}
...
;}
;break;}
;qx.Bootstrap.debug(this,f+l+j);delete this.__bW;}
...
},defer:function(H){if(window&&window.top){var frames=window.top.frames;for(var i=0;i<frames.length;i++ ){if(frames[i]===window){H.__ca=c+(i+1);return;}
;}
;}
;H.__ca=h;}
});}
...
(function(){var a='"',b="Class '",c="",d="Unsupported type!",e="change",f="qx.data.marshal.Json",g="Array",h="_validate",j="' could not be found.",k="]",l="[",m="set",n="_applyEventPropagation",o="qx.data.model.";qx.Class.define(f,{extend:qx.core.Object,implement:[qx.data.marshal.IMarshaler],construct:function(p){qx.core.Object.call(this);this.__tw=p;}
...
},members:{__tw:null,__tx:function(s){return Object.keys(s).sort().join(a);}
,toClass:function(t,u){this.__ty(t,u,null,0);}
,__ty:function(G,B,v,D){if(!qx.lang.Type.isObject(G)||!!G.$$isString||G instanceof qx.core.Object){if(G instanceof Array||qx.Bootstrap.getClass(G)==g){for(var i=0;i<G.length;i++ ){this.__ty(G[i],B,v+l+i+k,D+1);}
;}
;return;}
;var x=this.__tx(G);if(this.__tC(x,v,D)){return;}
;for(var F in G){this.__ty(G[F],B,F,D+1);}
;if(qx.Class.isDefined(o+x)){return;}
;if(this.__tw&&this.__tw.getModelClass&&this.__tw.getModelClass(x,G,v,D)!=null){return;}
;var H={};var y={__tz:this.__tz};for(var F in G){if(this.__tw&&this.__tw.getPropertyMapping){F=this.__tw.getPropertyMapping(F,x);}
;F=F.replace(/-|\.|\s+/g,c);{}
;H[F]={};H[F].nullable=true;H[F].event=e+qx.lang.String.firstUp(F);if(B){H[F].apply=n;}
;if(this.__tw&&this.__tw.getValidationRule){var A=this.__tw.getValidationRule(x,F);if(A){H[F].validate=h+F;y[h+F]=A;}
;}
;}
;if(this.__tw&&this.__tw.getModelSuperClass){var E=this.__tw.getModelSuperClass(x,v,D)||qx.core.Object;}
else {var E=qx.core.Object;}
;var z=[];if(this.__tw&&this.__tw.getModelMixins){var C=this.__tw.getModelMixins(x,v,D);if(!qx.lang.Type.isArray(C)){if(C!=null){z=[C];}
;}
else {z=C;}
;}
;if(B){z.push(qx.data.marshal.MEventBubbling);}
;var w={extend:E,include:z,properties:H,members:y,destruct:this.__tA};qx.Class.define(o+x,w);}
,__tA:function(){var I=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var J in I){this.__tz(this.get(I[J].name));}
;}
,__tz:function(K){if(!(K instanceof qx.core.Object)){return;}
;if(K.isDisposed()){return;}
;K.dispose();}
,__tB:function(L,P,Q,O){var R;if(this.__tw&&this.__tw.getModelClass){R=this.__tw.getModelClass(L,P,Q,O);}
;if(R!=null){return (new R());}
else {var N=o+L;var M=qx.Class.getByName(N);if(!M){throw new Error(b+N+j);}
;return (new M());}
;}
,__tC:function(S,V,T){var U=this.__tw;return U&&U.ignore&&U.ignore(S,V,T);}
,toModel:function(W){return this.__tD(W,null,0);}
,__tD:function(bj,Y,bf){var bd=qx.lang.Type.isObject(bj);var X=bj instanceof Array||qx.Bootstrap.getClass(bj)==g;if((!bd&&!X)||!!bj.$$isString||bj instanceof qx.core.Object){return bj;}
else if(this.__tC(this.__tx(bj),Y,bf)){return bj;}
else if(X){var bh=qx.data.Array;if(this.__tw&&this.__tw.getArrayClass){var be=this.__tw.getArrayClass(Y,bf);bh=be||bh;}
;var bk=new bh();bk.setAutoDisposeItems(true);for(var i=0;i<bj.length;i++ ){bk.push(this.__tD(bj[i],Y+l+i+k,bf+1));}
;return bk;}
else if(bd){var ba=this.__tx(bj);var bi=this.__tB(ba,bj,Y,bf);for(var bg in bj){var bb=bg;if(this.__tw&&this.__tw.getPropertyMapping){bb=this.__tw.getPropertyMapping(bg,ba);}
;var bl=bb.replace(/-|\.|\s+/g,c);{}
;bb=bl;var bc=m+qx.lang.String.firstUp(bb);if(bi[bc]){bi[bc](this.__tD(bj[bg],bg,bf+1));}
;}
;return bi;}
;throw new Error(d);}
...
;if((n instanceof qx.core.Object)&&qx.Class.hasMixin(n.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__lo,this,name);var q=n.addListener(g,o,this);var p=n.getUserData(d+this.$$hash);if(p==null){p=[];n.setUserData(d+this.$$hash,p);}
;p.push(q);}
;}
,__lo:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(h)!=-1?y.name.indexOf(h):y.name.length;var v=y.name.indexOf(f)!=-1?y.name.indexOf(f):y.name.length;if(v==0){var t=name+y.name;}
...
else if(arguments.length>1){this.__lp=[];for(var i=0;i<arguments.length;i++ ){this.__lp.push(arguments[i]);}
;}
else if(typeof r==n){this.__lp=new Array(r);}
else if(r instanceof Array){this.__lp=qx.lang.Array.clone(r);}
else {this.__lp=[];this.dispose();throw new Error(j);}
;for(var i=0;i<this.__lp.length;i++ ){this._applyEventPropagation(this.__lp[i],null,i);}
;this.__lq();{}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":q,"changeLength":q},members:{__lp:null,concat:function(s){if(s){var t=this.__lp.concat(s);}
else {var t=this.__lp.concat();}
;return new qx.data.Array(t);}
,join:function(u){return this.__lp.join(u);}
,pop:function(){var v=this.__lp.pop();this.__lq();this._registerEventChaining(null,v,this.length-1);this.fireDataEvent(o,{value:[],name:this.length+f,old:[v],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:l,removed:[v],added:[]},null);return v;}
,push:function(w){for(var i=0;i<arguments.length;i++ ){this.__lp.push(arguments[i]);this.__lq();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(o,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(g,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var x=this.__lp.concat();this.__lp.reverse();this.__lr(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__lp,name:k+(this.__lp.length-1),old:x,item:this});}
,shift:function(){if(this.length==0){return;}
;var y=this.__lp.shift();this.__lq();this._registerEventChaining(null,y,this.length-1);this.__lr(0,this.length);this.fireDataEvent(o,{value:[],name:m,old:[y],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:l,removed:[y],added:[]},null);return y;}
,slice:function(A,z){return new qx.data.Array(this.__lp.slice(A,z));}
,splice:function(F,H,J){var N=this.__lp.length;var I=this.__lp.splice.apply(this.__lp,arguments);if(this.__lp.length!=N){this.__lq();}
...
;this.__lr(F+(arguments.length-2)-H,this.length);if(L||D){var G=[];for(var i=2;i<arguments.length;i++ ){G[i-2]=arguments[i];}
...
;var O=this.__lp.concat();this.__lp.sort.apply(this.__lp,arguments);if(qx.lang.Array.equals(this.__lp,O)===true){return;}
;this.__lr(0,this.length);this.fireDataEvent(g,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(o,{value:this.__lp,name:k+(this.length-1),old:O,item:this});}
,unshift:function(Q){for(var i=arguments.length-1;i>=0;i-- ){this.__lp.unshift(arguments[i]);this.__lq();this.__lr(0,this.length);this.fireDataEvent(o,{value:[this.__lp[0]],name:m,old:[this.__lp[1]],item:this});this.fireDataEvent(g,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__lp;}
,getItem:function(R){return this.__lp[R];}
,setItem:function(S,U){var T=this.__lp[S];if(T===U){return;}
;this.__lp[S]=U;this._registerEventChaining(U,T,S);if(this.length!=this.__lp.length){this.__lq();}
...
,indexOf:function(V){return this.__lp.indexOf(V);}
,lastIndexOf:function(W){return this.__lp.lastIndexOf(W);}
,toString:function(){if(this.__lp!=null){return this.__lp.toString();}
;return f;}
,contains:function(X){return this.__lp.indexOf(X)!==-1;}
...
,removeAll:function(){for(var i=0;i<this.__lp.length;i++ ){this._registerEventChaining(null,this.__lp[i],i);}
;if(this.getLength()==0){return [];}
;var bl=this.getLength();var bk=this.__lp.concat();this.__lp.length=0;this.__lq();this.fireDataEvent(o,{value:[],name:k+(bl-1),old:bk,item:this});this.fireDataEvent(g,{start:0,end:bl-1,type:l,removed:bk,added:[]},null);return bk;}
,append:function(bm){if(bm instanceof qx.data.Array){bm=bm.toArray();}
;{}
;var bn=this.__lp.length;Array.prototype.push.apply(this.__lp,bm);for(var i=0;i<bm.length;i++ ){this._registerEventChaining(bm[i],null,bn+i);}
;var bn=this.length;this.__lq();var name=bn==(this.length-1)?bn:bn+a+(this.length-1);this.fireDataEvent(o,{value:bm,name:name+f,old:[],item:this});this.fireDataEvent(g,{start:bn,end:this.length-1,type:b,added:bm,removed:[]},null);}
...
,forEach:function(bu,bv){for(var i=0;i<this.__lp.length;i++ ){bu.call(bv,this.__lp[i],i,this);}
;}
,filter:function(bw,self){return new qx.data.Array(this.__lp.filter(bw,self));}
,map:function(bx,self){return new qx.data.Array(this.__lp.map(bx,self));}
,some:function(by,self){return this.__lp.some(by,self);}
,every:function(bz,self){return this.__lp.every(bz,self);}
,reduce:function(bB,bA){return this.__lp.reduce(bB,bA);}
,reduceRight:function(bD,bC){return this.__lp.reduceRight(bD,bC);}
,__lq:function(){var bE=this.length;this.length=this.__lp.length;this.fireDataEvent(p,this.length,bE);}
,__lr:function(bG,bF){for(var i=bG;i<bF;i++ ){this._registerEventChaining(this.__lp[i],this.__lp[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__lp.length;i++ ){var bH=this.__lp[i];this._applyEventPropagation(null,bH,i);if(this.isAutoDisposeItems()&&bH&&bH instanceof qx.core.Object){bH.dispose();}
;}
;this.__lp=null;}
});}
)();
